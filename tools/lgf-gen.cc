...
bool tsp_improve(Node u) {
  for (IncEdgeIt e(g, u); e != INVALID; ++e)
    if (tsp_improve(u, g.runningNode(e)))
      return true;
  return false;
}
...
struct YLess {
  ...
  bool operator()(const Part& l, const Part& r) const {
    ...
    if (lex < epsilon + rex && lbx + epsilon < rex)
      return true;
    if (rex < epsilon + lex && rbx + epsilon < lex)
      return false;
    return lex < rex;
  }
...
inline void delaunay() {
  ...
  int siteindex = 0;
  {
    ...
    while (siteindex < int(siteheap.size()) &&
      ...
      front.push_back(std::make_pair(
          points[siteheap[siteindex].second].y, siteheap[siteindex].second));
      ++siteindex;
    }
    ...
    for (int i = 0; i < int(front.size()); ++i) {
      ...
      beach.insert(std::make_pair(Part(prev, curr, next), spikeheap.end()));
    }
  }
  ...
  while (siteindex < int(points.size()) || !spikeheap.empty()) {
    ...
    if (siteindex < int(points.size()) &&
      ...
      if (prev != -1 && circle_form(points[prev], points[curr], points[site])) {
      ...
      if (next != -1 && circle_form(points[site], points[curr], points[next])) {
...

      {
        ...
        arc = prev < curr ? std::make_pair(prev, curr)
        ...
        arc = curr < next ? std::make_pair(curr, next)
                          : std::make_pair(next, curr);
      ...
      Beach::iterator pbit = bit;
      --pbit;
      int             ppv  = pbit->first.prev;
      Beach::iterator nbit = bit;
      ++nbit;
...
void sparseTriangle(int d) {
  ...
  for (std::vector<Parc>::iterator pi = pedges.begin(); pi != pedges.end();
       ++pi) {
    Line   li(pi->a, pi->b);
    EdgeIt e(g);
...
class LengthSquareMap {
 public:
  ...
  Value operator[](const Key& key) const {
    return (_coords[_graph.v(key)] - _coords[_graph.u(key)]).normSquare();
  }
...
int main(int argc, const char** argv) {
  ...
  ap.refOption("n", "Number of nodes (default is 100)", N)
      ...
      .refOption(
          "area", "Full relative area of the cities (default is 1)", area)
      .refOption(
          "disc",
      ...
      .refOption(
          "square", "Nodes are evenly distributed on a unit square", square_d)
      .optionGroup("dist", "square")
      .refOption(
          "gauss",
          "Nodes are located according to a two-dim Gauss "
          "distribution",
          gauss_d)
      ...
      .boolOption(
          "dir",
          "Directed graph is generated (each edge is replaced by "
          "two directed arcs)")
  ...
  for (int s = 0; s < num_of_cities; s++) {
    ...
    if (gauss_d)
      for (; i < N * (cum_sizes[s] / sum_sizes); i++) {
        ...
        nodes.push_back(n);
        coords[n] =
            center + rnd.gauss2() * area * std::sqrt(sizes[s] / sum_sizes);
      }
    else if (square_d)
      for (; i < N * (cum_sizes[s] / sum_sizes); i++) {
        Node n = g.addNode();
      ...
      for (; i < N * (cum_sizes[s] / sum_sizes); i++) {
        ...
        nodes.push_back(n);
        coords[n] =
            center + rnd.disc() * area * std::sqrt(sizes[s] / sum_sizes);
      }
  }
  ...
  if (ap["dir"])
    ...
    GraphWriter<ListGraph>(g, prefix + ".lgf")
        .nodeMap("coordinates_x", scaleMap(xMap(coords), 600))
        .nodeMap("coordinates_y", scaleMap(yMap(coords), 600))
        .run();
}
