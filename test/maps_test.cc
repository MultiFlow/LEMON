...
inline bool operator<(C c1, C c2) {
...
int main() {
  ...
  // ConstMap
  {
    ...
    check(
        map6[A()] == 10 && map7[A()] == 10, "Something is wrong with ConstMap");
  }
  ...
  // RangeMap
  {
    ...
    checkConcept<
        ReferenceMap<int, double, double&, const double&>,
        RangeMap<double>>();
    ...
    check(
        map9.size() == 10 && map9[2] == 0 && map9[5] == 100,
  ...
  // SparseMap
  {
    ...
    checkConcept<
        ReferenceMap<double, int, int&, const int&>,
    ...
    check(
        map5[1.0] == 0 && map5[3.14] == 0 && map6[1.0] == 10 &&
            map6[3.14] == 10,
        "Something is wrong with SparseMap");
    map5[1.0] = map6[3.14] = 100;
    check(
        map5[1.0] == 100 && map5[3.14] == 0 && map6[1.0] == 10 &&
            map6[3.14] == 100,
        "Something is wrong with SparseMap");
  }
  ...
  // ComposeMap
  {
    ...
    m1[3.14] = true;
    RangeMap<double> m2(2);
    m2[0] = 3.0;
    m2[1] = 3.14;
    check(
        !composeMap(m1, m2)[0] && composeMap(m1, m2)[1],
  ...
  // CombineMap
  {
    ...
    check(
        combineMap(constMap<B, int, 2>(), identityMap<B>(), &binc)[B()] == 3,
  ...
  // FunctorToMap, MapToFunctor
  {
    ...
    check(
        functorToMap(&func)[A()] == 3, "Something is wrong with FunctorToMap");
    check(
        mapToFunctor(constMap<A, int>(2))(A()) == 2,
    ...
    check(
        functorToMap(mapToFunctor(constMap<A, int>(2)))[A()] == 2,
        "Something is wrong with FunctorToMap or MapToFunctor");
  }
  ...
  // - NegMap, NegWriteMap, AbsMap
  {
    ...
    check(
        addMap(c1, id)[0] == 1.0 && addMap(c1, id)[10] == 11.0,
        "Something is wrong with AddMap");
    check(
        subMap(id, c1)[0] == -1.0 && subMap(id, c1)[10] == 9.0,
        "Something is wrong with SubMap");
    check(
        mulMap(id, c2)[0] == 0 && mulMap(id, c2)[2] == 6.28,
        "Something is wrong with MulMap");
    check(
        divMap(c2, id)[1] == 3.14 && divMap(c2, id)[2] == 1.57,
    ...
    check(
        shiftMap(id, 2.0)[1] == 3.0 && shiftMap(id, 2.0)[10] == 12.0,
        "Something is wrong with ShiftMap");
    check(
        shiftWriteMap(id, 2.0)[1] == 3.0 && shiftWriteMap(id, 2.0)[10] == 12.0,
        "Something is wrong with ShiftWriteMap");
    check(
        scaleMap(id, 2.0)[1] == 2.0 && scaleMap(id, 2.0)[10] == 20.0,
        "Something is wrong with ScaleMap");
    check(
        scaleWriteMap(id, 2.0)[1] == 2.0 && scaleWriteMap(id, 2.0)[10] == 20.0,
        "Something is wrong with ScaleWriteMap");
    check(
        negMap(id)[1] == -1.0 && negMap(id)[-10] == 10.0,
        "Something is wrong with NegMap");
    check(
        negWriteMap(id)[1] == -1.0 && negWriteMap(id)[-10] == 10.0,
        "Something is wrong with NegWriteMap");
    check(
        absMap(id)[1] == 1.0 && absMap(id)[-10] == 10.0,
  ...
  // - EqualMap, LessMap
  {
    ...
    rm[1] = false;
    check(
        andMap(tm, rm)[0] && !andMap(tm, rm)[1] && !andMap(fm, rm)[0] &&
            !andMap(fm, rm)[1],
        "Something is wrong with AndMap");
    check(
        orMap(tm, rm)[0] && orMap(tm, rm)[1] && orMap(fm, rm)[0] &&
            !orMap(fm, rm)[1],
        "Something is wrong with OrMap");
    check(!notMap(rm)[0] && notMap(rm)[1], "Something is wrong with NotMap");
    check(
        !notWriteMap(rm)[0] && notWriteMap(rm)[1],
    ...
    ConvertMap<IdentityMap<int>, double> id(im);
    check(
        lessMap(id, cm)[1] && !lessMap(id, cm)[2] && !lessMap(id, cm)[3],
        "Something is wrong with LessMap");
    check(
        !equalMap(id, cm)[1] && equalMap(id, cm)[2] && !equalMap(id, cm)[3],
  ...
  // LoggerBoolMap
  {
    ...
    map2.set(60, true);
    check(
        v1.size() == 3 && v2.size() == 10 && v1[0] == 20 && v1[1] == 50 &&
            v1[2] == 60 && v2[0] == 20 && v2[1] == 50 && v2[2] == 60,
...

    {
      ...
      check(
          v.size() == 4 && v[0] == n1 && v[1] == n2 && v[2] == n0 && v[3] == n3,
          "Something is wrong with LoggerBoolMap");
    }
    {
  ...
  // IdMap, RangeIdMap
  {
    ...
    check(nrmap.size() == 3 && armap.size() == 4, "Wrong RangeIdMap::size()");
    ...
    check(nrmap.size() == 2 && armap.size() == 2, "Wrong RangeIdMap::size()");
  ...
  // CrossRefMap
  {
    ...
    checkConcept<ReadWriteMap<Node, int>, CrossRefMap<Graph, Node, int>>();
    checkConcept<ReadWriteMap<Node, bool>, CrossRefMap<Graph, Node, bool>>();
    checkConcept<
        ReadWriteMap<Node, double>,
    ...
    check(
        map[n0] == 'A' && map('A') == n0 && map.inverse()['A'] == n0,
        "Wrong CrossRefMap");
    check(
        map[n1] == 'B' && map('B') == n1 && map.inverse()['B'] == n1,
        "Wrong CrossRefMap");
    check(
        map[n2] == 'C' && map('C') == n2 && map.inverse()['C'] == n2,
        "Wrong CrossRefMap");
    check(
        map.count('A') == 1 && map.count('B') == 1 && map.count('C') == 1,
        "Wrong CrossRefMap::count()");

    CRMap::ValueIt it = map.beginValue();
    check(
        *it++ == 'A' && *it++ == 'B' && *it++ == 'C' && it == map.endValue(),
        "Wrong value iterator");

    map.set(n2, 'A');

    check(
        map[n0] == 'A' && map[n1] == 'B' && map[n2] == 'A',
        "Wrong CrossRefMap");
    check(map('A') == n0 && map.inverse()['A'] == n0, "Wrong CrossRefMap");
    check(map('B') == n1 && map.inverse()['B'] == n1, "Wrong CrossRefMap");
    check(
        map('C') == INVALID && map.inverse()['C'] == INVALID,
        "Wrong CrossRefMap");
    check(
        map.count('A') == 2 && map.count('B') == 1 && map.count('C') == 0,
        "Wrong CrossRefMap::count()");

    it = map.beginValue();
    check(
        *it++ == 'A' && *it++ == 'A' && *it++ == 'B' && it == map.endValue(),
        "Wrong value iterator");

    map.set(n0, 'C');

    check(
        map[n0] == 'C' && map[n1] == 'B' && map[n2] == 'A',
        "Wrong CrossRefMap");
  ...
  // Iterable int map
  {
    ...
    {
      int c = 0;
      for (auto v : map1.items(0)) {
        c++;
        ::lemon::ignore_unused_variable_warning(v);
      }
      check(c == (num + 1) / 2, "Wrong number");
      for (auto v : map1.items(1)) {
        c++;
        ::lemon::ignore_unused_variable_warning(v);
      }
  ...
  // Iterable value map
  {
    ...
    for (Ivm::ValueIt vit = map1.beginValue(); vit != map1.endValue(); ++vit) {
      check(
          map1[static_cast<Item>(Ivm::ItemIt(map1, *vit))] == *vit,
          "Wrong ValueIt");
    }
    ...
    {
      int c = 0;
      for (auto v : map1.items(0.0)) {
        c++;
        ::lemon::ignore_unused_variable_warning(v);
      }
      check(c == (num + 1) / 2, "Wrong number");
      for (auto v : map1.items(1.0)) {
        c++;
        ::lemon::ignore_unused_variable_warning(v);
      }
      check(c == num, "Wrong number");
  ...
  // mapCopy(), mapCompare(), mapFill()
  {
    ...
    check(
        mapMinValue(g, map2, std::greater<int>()) == 'c',
        "Wrong mapMinValue()");
    check(
        mapMaxValue(g, map2, std::greater<int>()) == 'a',
        "Wrong mapMaxValue()");
    ...
    check(
        mapMin(g, composeMap(functorToMap(&createC), map2)) == a2,
        "Wrong mapMin()");
    check(
        mapMax(g, composeMap(functorToMap(&createC), map2)) == a4,
        "Wrong mapMax()");
    check(
        mapMinValue(g, composeMap(functorToMap(&createC), map2)) == C('a'),
        "Wrong mapMinValue()");
    check(
        mapMaxValue(g, composeMap(functorToMap(&createC), map2)) == C('c'),
        "Wrong mapMaxValue()");
    ...
    check(mapFindIf(g, map1, Less<int>(7)) == n2, "Wrong mapFindIf()");
    check(mapFindIf(g, map1, Less<int>(5)) == INVALID, "Wrong mapFindIf()");
    check(mapFindIf(g, map2, Less<char>('d')) == ArcIt(g), "Wrong mapFindIf()");
    check(mapFindIf(g, map2, Less<char>('a')) == INVALID, "Wrong mapFindIf()");
    ...
    check(mapCountIf(g, map1, Less<int>(11)) == 2, "Wrong mapCountIf()");
    check(mapCountIf(g, map1, Less<int>(13)) == 3, "Wrong mapCountIf()");
    check(mapCountIf(g, map1, Less<int>(5)) == 0, "Wrong mapCountIf()");
    check(mapCountIf(g, map2, Less<char>('d')) == 4, "Wrong mapCountIf()");
    check(mapCountIf(g, map2, Less<char>('c')) == 3, "Wrong mapCountIf()");
    check(mapCountIf(g, map2, Less<char>('a')) == 0, "Wrong mapCountIf()");
