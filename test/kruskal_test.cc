...
int main() {
  ...
  // Test with const map.
  check(
      kruskal(G, ConstMap<ListGraph::Edge, int>(2), tree_map) == 10,
  ...
  check(
      kruskal(G, edge_cost_map, tree_edge_vec.begin()) == -31,
      "Total cost should be -31.");

  tree_edge_vec.clear();

  check(
      kruskal(G, edge_cost_map, back_inserter(tree_edge_vec)) == -31,
      "Total cost should be -31.");
  ...
  check(
      tree_edge_vec[0] == e1 && tree_edge_vec[1] == e2 &&
          tree_edge_vec[2] == e5 && tree_edge_vec[3] == e7 &&
          tree_edge_vec[4] == e9,
      "Wrong tree.");
