...
void checkGraphArcList(const Graph& G, int cnt) {
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    check(G.oppositeNode(G.source(e), e) == G.target(e), "Wrong opposite node");
    check(G.oppositeNode(G.target(e), e) == G.source(e), "Wrong opposite node");
    ++e;
  }
  check(e == INVALID, "Wrong Arc list linking.");
...
void checkGraphIncEdgeList(const Graph& G, typename Graph::Node n, int cnt) {
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    check(n == G.baseNode(e), "Wrong OutArc list linking.");
    check(
        G.u(e) == G.runningNode(e) || G.v(e) == G.runningNode(e),
...
template<class Graph>
void checkGraphIncEdgeArcLists(
    const Graph&         G,
