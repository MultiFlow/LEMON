...
bool checkTourPath(const FullGraph& gr, const Path<FullGraph>& p) {
  ...
  if (!checkPath(gr, p))
    return false;
  if (gr.nodeNum() <= 1 && p.length() != 0)
    return false;
  if (gr.nodeNum() > 1 && p.length() != gr.nodeNum())
    return false;

  for (int i = 0; i < p.length(); ++i) {
    if (used[gr.target(p.nth(i))])
      return false;
...
bool checkCost(
    const FullGraph&                    gr,
    const std::vector<FullGraph::Node>& p,
    const CostMap&                      cost,
...
void tspTestSmall(const std::string& alg_name) {
  ...
  for (int n = 0; n <= 5; ++n) {
    ...
    check(
        checkCost(g, vec1, constMap<Edge, int>(1), esize),
    ...
    check(
        checkCost(g, path, constMap<Edge, int>(1), esize),
        alg_name + ": Wrong tour cost");
  }
}
...
void tspTestRandom(const std::string& alg_name) {
  ...
  for (int i = 1; i <= 3; i++) {
    ...
    check(
        checkCost(g, vec, cost, alg.tourCost()),
        alg_name + ": Wrong tour cost");
    ...
    check(checkTourPath(g, path), alg_name + ": Wrong tour");
    check(
        checkCost(g, path, cost, alg.tourCost()),
        alg_name + ": Wrong tour cost");

    check(
        !Tolerance<double>().less(alg.tourCost(), opt2.run(alg.tourNodes())),
        "2-opt improvement: Wrong total cost");
    check(
        checkTour(g, opt2.tourNodes()),
        "2-opt improvement: Wrong node sequence");
    check(
        checkCost(g, opt2.tourNodes(), cost, opt2.tourCost()),
        "2-opt improvement: Wrong tour cost");

    check(
        !Tolerance<double>().less(alg.tourCost(), opt2.run(path)),
        "2-opt improvement: Wrong total cost");
    check(
        checkTour(g, opt2.tourNodes()),
        "2-opt improvement: Wrong node sequence");
