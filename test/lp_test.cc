...
int countCols(LpBase& lp) {
  ...
  for (auto c : lp.cols()) {
    cnt++;
    ::lemon::ignore_unused_variable_warning(c);
  }
...
int countRows(LpBase& lp) {
  int count = 0;
  for (LpBase::RowIt r(lp); r != INVALID; ++r)
    ++count;
#ifdef LEMON_CXX11
  int cnt = 0;
  for (auto r : lp.rows()) {
    cnt++;
    ::lemon::ignore_unused_variable_warning(r);
  }
  check(count == cnt, "Wrong STL iterator");
#endif
  return count;
...
void lpTest(LpSolver& lp) {
...

  {
    ...
    *e -= 12;
    ...
    e =
        ((p1 + p2) + (p1 - p2) + (p1 + 12) + (12 + p1) + (p1 - 12) + (12 - p1) +
         (f + 12) + (12 + f) + (p1 + f) + (f + p1) + (f + g) + (f - 12) +
         (12 - f) + (p1 - f) + (f - p1) + (f - g) + 2.2 * f + f * 2.2 +
         f / 2.2 + 2 * f + f * 2 + f / 2 + 2.2 * p1 + p1 * 2.2 + p1 / 2.2 +
         2 * p1 + p1 * 2 + p1 / 2);
    ...
    *e      = 12;
    ...
    lp.addRow(
        -LP::INF,
        3.0 * (x[1] + x[2] * 2 - 5 * x[3] + 12 - x[4] / 3) + 2 * x[4] - 4,
        23);
...

  {
    ...
    e =
        ((p1 + p2) + (p1 - p2) + (p1 + f) + (f + p1) + (f + g) + (p1 - f) +
         (f - p1) + (f - g) + 2.2 * f + f * 2.2 + f / 2.2 + 2 * f + f * 2 +
         f / 2 + 2.2 * p1 + p1 * 2.2 + p1 / 2.2 + 2 * p1 + p1 * 2 + p1 / 2);
  }
}

void solveAndCheck(LpSolver& lp, LpSolver::ProblemType stat, double exp_opt) {
  using std::string;
  lp.solve();
...
void aTest(LpSolver& lp) {
  ...
  check(
      lp.colUpperBound(x1) == LpSolver::INF,
      "The upper bound for variable x1 should be infty.");
  check(
      lp.rowLowerBound(upright) == -LpSolver::INF,
      "The lower bound for the first row should be -infty.");
  check(
      lp.rowUpperBound(upright) == 1,
  ...
  check(
      clp->colLowerBound(x1) == 0,
      "The lower bound for variable x1 should be 0.");
  check(
      clp->colUpperBound(x1) == LpSolver::INF,
      "The upper bound for variable x1 should be infty.");
...
void rangeConstraintTest() {
  ...
  check(
      lp.primal() <= 67.501 && lp.primal() >= 67.499, "Wrong objective value");
  check(lp.primal(x1) <= 7.501 && lp.primal(x1) >= 7.499, "Wrong value for x1");
  check(
      lp.primal(x2) <= 10.001 && lp.primal(x2) >= 9.999, "Wrong value for x2");
}
