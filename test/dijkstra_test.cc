...
void checkDijkstraCompile() {
  ...
  {
    DType ::SetPredMap<concepts::ReadWriteMap<Node, Arc>>::SetDistMap<
        concepts::ReadWriteMap<Node, VType>>::SetStandardProcessedMap ::
        SetProcessedMap<concepts::WriteMap<Node, bool>>::
            SetOperationTraits<DijkstraDefaultOperationTraits<VType>>::SetHeap<
                BinHeap<VType, concepts::ReadWriteMap<Node, int>>>::
                SetStandardHeap<
                    BinHeap<VType, concepts::ReadWriteMap<Node, int>>>::
                    SetHeap<
                        BinHeap<VType, concepts::ReadWriteMap<Node, int>>,
                        concepts::ReadWriteMap<Node, int>>::Create
    ...
    dijkstra_test.lengthMap(length_map)
        .predMap(pred_map)
        .distMap(dist_map)
...
void checkDijkstra() {
  ...
  Dijkstra<Digraph, LengthMap> dijkstra_test(G, length);
  dijkstra_test.run(s);
  ...
  for (NodeIt v(G); v != INVALID; ++v) {
    if (dijkstra_test.reached(v)) {
      ...
      if (dijkstra_test.predArc(v) != INVALID) {
        ...
        check(
            dijkstra_test.dist(v) - dijkstra_test.dist(u) == length[e],
            "Wrong distance! Difference: " << std::abs(
                dijkstra_test.dist(v) - dijkstra_test.dist(u) - length[e]));
      }
    }
  }
