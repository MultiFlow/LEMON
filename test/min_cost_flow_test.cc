...
template<typename GR, typename LM, typename UM, typename SM, typename FM>
bool checkFlow(
    const GR&  gr,
    const LM&  lower,
    const UM&  upper,
...
bool checkPotential(
  ...
  for (ArcIt e(gr); opt && e != INVALID; ++e) {
    typename CM::Value red_cost = cost[e] + pi[gr.source(e)] - pi[gr.target(e)];
    opt = red_cost == 0 || (red_cost > 0 && flow[e] == lower[e]) ||
...
bool checkDualCost(
  ...
  for (ArcIt a(gr); a != INVALID; ++a) {
    typename CM::Value red_cost = cost[a] + pi[gr.source(a)] - pi[gr.target(a)];
...
void checkMcf(
  ...
  if (optimal) {
    ...
    mcf.potentialMap(pi);
    check(
        checkFlow(gr, lower, upper, supply, flow, type),
        "The flow is not feasible " + test_id);
    check(mcf.totalCost() == total, "The flow is not optimal " + test_id);
    check(
        checkPotential(gr, lower, upper, cost, supply, flow, pi, type),
        "Wrong potentials " + test_id);
    check(
        checkDualCost(gr, lower, upper, cost, supply, pi, total),
...
template<typename MCF, typename Param>
void runMcfLeqTests(Param param, const std::string& test_str = "") {
...
int main() {
  ...
  // Check the interface of NetworkSimplex
  {
    ...
    checkConcept<McfClassConcept<GR, int, int>, NetworkSimplex<GR>>();
    checkConcept<
        McfClassConcept<GR, double, double>,
        NetworkSimplex<GR, double>>();
    checkConcept<
        McfClassConcept<GR, int, double>,
  ...
  {
    typedef concepts::Digraph GR;
    checkConcept<McfClassConcept<GR, int, int>, CapacityScaling<GR>>();
    checkConcept<
        McfClassConcept<GR, double, double>,
    ...
    typedef CapacityScaling<GR>::SetHeap<
        concepts::Heap<int, RangeMap<int>>>::Create CAS;
  ...
  {
    typedef concepts::Digraph GR;
    checkConcept<McfClassConcept<GR, int, int>, CostScaling<GR>>();
    checkConcept<
        McfClassConcept<GR, double, double>,
    ...
    typedef CostScaling<GR>::SetLargeCost<double>::Create COS;
  ...
  {
    typedef concepts::Digraph GR;
    checkConcept<McfClassConcept<GR, int, int>, CycleCanceling<GR>>();
    checkConcept<
        McfClassConcept<GR, double, double>,
