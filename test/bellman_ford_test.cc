...
void checkBellmanFordCompile() {
...

  {
    ...
    for (auto n : const_bf_test.activeNodes()) {
  ...
  }
  {
    BF::SetPredMap<concepts::ReadWriteMap<Node, Arc>>::
        SetDistMap<concepts::ReadWriteMap<Node, Value>>::SetOperationTraits<
            BellmanFordDefaultOperationTraits<Value>>::Create
            bf_test(gr, length);
    ...
    bf_test.lengthMap(length_map).predMap(pred_map).distMap(dist_map);
...
void checkBellmanFord() {
  ...
  BellmanFord<Digraph, LengthMap> bf(gr, length);
  bf.run(s);
  ...
  for (ArcIt e(gr); e != INVALID; ++e) {
    ...
    Node v = gr.target(e);
    check(
        !bf.reached(u) || (bf.dist(v) - bf.dist(u) <= length[e]),
  ...
  for (NodeIt v(gr); v != INVALID; ++v) {
    if (bf.reached(v)) {
      ...
      if (bf.predArc(v) != INVALID) {
        ...
        check(
            bf.dist(v) - bf.dist(u) == length[e],
            "Wrong distance! Difference: " << bf.dist(v) - bf.dist(u) -
                                                  length[e]);
      }
    }
  }
...
void checkBellmanFordNegativeCycle() {
...

  {
    ...
    check(
        p.length() == 1 && p.front() == p.back() && p.front() == a2,
...

  {
    BellmanFord<SmartDigraph, IntArcMap> bf(gr, length);
    bf.run(n1);
    check(bf.negativeCycle().empty(), "Negative cycle should not be found.");
  }
...

  {
    ...
    check(
        length[p.nth(0)] + length[p.nth(1)] + length[p.nth(2)] == -1,
        "Wrong negative cycle.");
  }
}
