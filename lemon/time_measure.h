...
class TimeStamp {
 ...
 private:
  ...
  void _reset() { utime = stime = cutime = cstime = rtime = 0; }
 ...
 public:
  ...
  ///
  double cUserTime() const { return cutime; }
  /// Gives back the user time of the process' children

  ///\note On <tt>WIN32</tt> platform this value is not calculated.
  ///
  double cSystemTime() const { return cstime; }
...
inline std::ostream& operator<<(std::ostream& os, const TimeStamp& t) {
  switch (t._format) {
    case TimeStamp::NORMAL:
      os << "u: " << t.userTime() << "s, s: " << t.systemTime()
         << "s, cu: " << t.cUserTime() << "s, cs: " << t.cSystemTime()
         << "s, real: " << t.realTime() << "s";
      break;
    case TimeStamp::SHORT:
      double total =
          t.userTime() + t.systemTime() + t.cUserTime() + t.cSystemTime();
      os << t.realTime() << "s (err: "
         << round((t.realTime() - total) / t.realTime() * 10000) / 100 << "%)";
      break;
  }
  return os;
}
...
class Timer {
 ...
 public:
  ...
  double userTime() const { return operator TimeStamp().userTime(); }
  /// Gives back the ellapsed system time of the process
  double systemTime() const { return operator TimeStamp().systemTime(); }
  /// Gives back the ellapsed user time of the process' children

  ///\note On <tt>WIN32</tt> platform this value is not calculated.
  ///
  double cUserTime() const { return operator TimeStamp().cUserTime(); }
  /// Gives back the ellapsed user time of the process' children

  ///\note On <tt>WIN32</tt> platform this value is not calculated.
  ///
  double cSystemTime() const { return operator TimeStamp().cSystemTime(); }
  /// Gives back the ellapsed real time
  double realTime() const { return operator TimeStamp().realTime(); }
...
TimeStamp runningTimeTest(
  ...
  Timer        t;
  for (unsigned int tn = 1; tn <= 1U << 31 && full.realTime() <= min_time;
       tn *= 2) {
    for (; total < tn; total++)
      f();
    full = t;
