...
class Opt2Tsp {
 ...
 public:
  ...
  Cost run(const Path& tour) {
    ...
    int                  first = _gr.id(_gr.source(it)), prev = first,
        curr = _gr.id(_gr.target(it)), next = -1;
  ...
  Cost run(const std::vector<Node>& tour) {
    ...
    int first = _gr.id(*it), prev = first, curr = _gr.id(*(++it)), next = -1;
    _plist[2 * first + 1] = curr;
  ...
  ///
  /// \pre run() must be called before using this function.
  const std::vector<Node>& tourNodes() const { return _path; }
 ...
 private:
  ...
  bool checkOpt2(const PathListIt& i, const PathListIt& j) {
    Node u = _gr.nodeFromId(i), un = _gr.nodeFromId(i.next()),
         v = _gr.nodeFromId(j), vn = _gr.nodeFromId(j.next());
