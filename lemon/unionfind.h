...
class UnionFind {
 ...
 private:
  ...
  bool rep(int idx) const { return items[idx] < 0; }
 ...
 public:
  ...
  ///
  int find(const Item& a) { return repIndex(index[a]); }
...
class UnionFindEnum {
 ...
 private:
  ...
  bool rep(int idx) const { return items[idx].parent < 0; }
  ...
  int classIndex(int idx) const { return ~(items[repIndex(idx)].parent); }
 ...
 public:
  ...
  int size(int cls) const { return classes[cls].size; }
  ...
  Item item(int cls) const { return items[classes[cls].firstItem].item; }
  ...
  class ClassIt {
   public:
    ...
    operator int() const { return cdx; }

    /// \brief Equality operator
    ///
    /// Equality operator
    bool operator==(const ClassIt& i) { return i.cdx == cdx; }
  ...
  class ItemIt {
   public:
    ...
    operator const Item&() const { return unionFind->items[idx].item; }

    /// \brief Equality operator
    ///
    /// Equality operator
    bool operator==(const ItemIt& i) { return i.idx == idx; }
...
class ExtendFindEnum {
 ...
 public:
  ...
  int find(const Item& item) const { return items[index[item]].cls; }
  ...
  class ItemIt {
   public:
    ...
    ///
    /// It converts the iterator to the current item.
    operator const Item&() const { return extendFind->items[idx].item; }
...
class HeapUnionFind {
      ...
      if (nodes[jd].size == 1) {
        if (nodes[jd].parent < 0) {
          ...
          if (nodes[nodes[jd].next].size < cmax) {
            ...
            if (less(jd, nodes[jd].next) || nodes[jd].item == nodes[pd].item) {
            ...
            pushRight(jd, ld);
            if (less(ld, nodes[jd].left) || nodes[ld].item == nodes[pd].item) {
      ...
      if (nodes[jd].size == 1) {
        if (nodes[jd].parent < 0) {
          ...
          if (nodes[nodes[jd].prev].size < cmax) {
            ...
            if (less(jd, nodes[jd].prev) || nodes[jd].item == nodes[pd].item) {
            ...
            pushLeft(jd, ld);
            if (less(ld, nodes[jd].right) || nodes[ld].item == nodes[pd].item) {
              nodes[jd].item = nodes[ld].item;
              nodes[jd].prio = nodes[ld].prio;
            }
 ...
 public:
  ...
  bool alive(int cls) const { return classes[cls].parent < 0; }
  ...
  bool trivial(int cls) const { return classes[cls].left == -1; }
  ...
  int find(const Item& item) const { return findClass(index[item]); }
  ...
  class ItemIt {
   ...
   public:
    ...
    ///
    /// It converts the iterator to the current item.
    operator const Item&() const { return _huf->nodes[_id].item; }

    /// \brief Equality operator
    ///
    /// Equality operator
    bool operator==(const ItemIt& i) { return i._id == _id; }
    ...
    ///
    /// Equality operator
    bool operator==(Invalid) { return _id == _lid; }
  ...
  class ClassIt {
   ...
   public:
    ClassIt(const HeapUnionFind& huf) : _huf(&huf), _id(huf.first_class) {}
    ...
    ///
    /// Equality operator
    bool operator==(const ClassIt& i) { return i._id == _id; }
