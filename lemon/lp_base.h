...
class LpBase {
 ...
 public:
  ...
  class Expr {
   ...
   public:
    ...
    /// Returns the coefficient of the column
    Value& operator[](const Col& c) { return comps[id(c)]; }
    ...
    void simplify(Value epsilon = 0.0) {
      ...
      if (std::fabs(const_comp) <= epsilon)
        const_comp = 0;
    }
    ...
    Expr& operator*=(const Value& v) {
      for (std::map<int, Value>::iterator it = comps.begin(); it != comps.end();
           ++it)
        it->second *= v;
      const_comp *= v;
      return *this;
    ...
    class CoeffIt {
     ...
     public:
      ...
      ///
      CoeffIt(Expr& e) : _it(e.comps.begin()), _end(e.comps.end()) {}

      /// Convert the iterator to the column of the term
      operator Col() const { return colFromId(_it->first); }
    ...
    class ConstCoeffIt {
     ...
     public:
      ...
      /// Sets the iterator to the first term of the expression.
      ///
      ConstCoeffIt(const Expr& e) : _it(e.comps.begin()), _end(e.comps.end()) {}
  ...
  class Constr {
   ...
   public:
    ...
    ///\e
    Constr(Value lb, const Expr& e, Value ub) : _expr(e), _lb(lb), _ub(ub) {}
    Constr(const Expr& e) : _expr(e), _lb(NaN), _ub(NaN) {}
    ///\e
    void clear() {
      _expr.clear();
      _lb = _ub = NaN;
    ...
    bool lowerBounded() const { return _lb != -INF && !isNaN(_lb); }
    /// Is the constraint upper bounded?
    bool upperBounded() const { return _ub != INF && !isNaN(_ub); }
  };
  ...
  class DualExpr {
   ...
   public:
    ...
    /// Returns the coefficient of the row
    Value& operator[](const Row& r) { return comps[id(r)]; }
    ...
    class CoeffIt {
     ...
     public:
      ...
      /// Sets the iterator to the first term of the expression.
      ///
      CoeffIt(DualExpr& e) : _it(e.comps.begin()), _end(e.comps.end()) {}

      /// Convert the iterator to the row of the term
      operator Row() const { return rowFromId(_it->first); }
 ...
 protected:
  ...
  class ExprIterator {
   ...
   public:
    ...
    ExprIterator(
        const std::map<int, Value>::const_iterator& host_it,
        const _solver_bits::VarIndex&               index)
   ...
   public:
    ...
    virtual const char* what() const throw() {
      ...
      if (!_what.empty())
        ...
        return "lemon::UnsupportedFormatError";
    }
  };
 ...
 public:
...
#else
  template<class T>
  typename enable_if<typename T::value_type::LpCol, int>::type addColSet(
      T& t,
      dummy<0> = 0) {
    int s = 0;
  ...
  template<class T>
  typename enable_if<typename T::value_type::second_type::LpCol, int>::type
  ...
  template<class T>
  typename enable_if<typename T::MapIt::Value::LpCol, int>::type addColSet(
      T& t,
      dummy<2> = 2) {
    int s = 0;
  ...
  void col(Col c, const DualExpr& e) {
    ...
    _setColCoeffs(
        _cols(id(c)),
        ExprIterator(e.comps.begin(), _rows),
...
#else
  template<class T>
  typename enable_if<typename T::value_type::LpRow, int>::type addRowSet(
      T& t,
      dummy<0> = 0) {
    int s = 0;
    for (typename T::iterator i = t.begin(); i != t.end(); ++i) {
  ...
  template<class T>
  typename enable_if<typename T::MapIt::Value::LpRow, int>::type addRowSet(
      T& t,
      dummy<2> = 2) {
    int s = 0;
    for (typename T::MapIt i(t); i != INVALID; ++i) {
  ...
  void row(Row r, Value l, const Expr& e, Value u) {
    ...
    _setRowCoeffs(
        _rows(id(r)),
        ExprIterator(e.comps.begin(), _cols),
  ...
  Row addRow(const Constr& c) {
    ...
    c.expr().simplify();
    r._id = _addRowId(_addRow(
        c.lowerBounded() ? c.lowerBound() - *c.expr() : -INF,
  ...
  Value colLowerBound(Col c) const { return _getColLowerBound(_cols(id(c))); }
  ...
  Value colUpperBound(Col c) const { return _getColUpperBound(_cols(id(c))); }
  ...
  template<class T1>
  typename enable_if<typename T1::value_type::second_type::LpCol, void>::type
  ...
  template<class T1>
  typename enable_if<typename T1::MapIt::Value::LpCol, void>::type
  ...
  Value rowLowerBound(Row r) const { return _getRowLowerBound(_rows(id(r))); }
  ...
  Value rowUpperBound(Row r) const { return _getRowUpperBound(_rows(id(r))); }
...
///
inline LpBase::Constr operator<=(const LpBase::Expr& e, const LpBase::Expr& f) {
...
///\relates LpBase::Constr
///
inline LpBase::Constr operator<=(
    const LpBase::Value& e,
    const LpBase::Expr&  f) {
...
///
inline LpBase::DualExpr operator+(
    const LpBase::DualExpr& a,
...
class LpSolver : virtual public LpBase {
 ...
 public:
  ...
  ProblemType primalType() const { return _getPrimalType(); }

  /// The type of the dual problem
  ProblemType dualType() const { return _getDualType(); }
...
class MipSolver : virtual public LpBase {
 public:
  ...
  /// Gives back the type of the column.
  ///
  ColTypes colType(Col c) const { return _getColType(_cols(id(c))); }
  ///@}
  ...
  ProblemType type() const { return _getType(); }
