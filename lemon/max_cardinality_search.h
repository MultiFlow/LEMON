...
struct MaxCardinalitySearchDefaultTraits {
  ...
  typedef typename HeapSelector<
      CapacityMap>::template Selector<Value, HeapCrossRef>::Heap Heap;
...
template<
    typename GR,
    typename CAP = ConstMap<typename GR::Arc, Const<int, 1>>,
    typename TR = MaxCardinalitySearchDefaultTraits<GR, CAP>>
#endif
class MaxCardinalitySearch {
 ...
 public:
  ...
  /// for the algorithm.
  template<class T>
  struct SetCapacityMap : public MaxCardinalitySearch<
                              Digraph,
                              CapacityMap,
                              DefCapacityMapTraits<T>> {
    typedef MaxCardinalitySearch<Digraph, CapacityMap, DefCapacityMapTraits<T>>
        Create;
  };
  ...
  template<class T>
  struct SetCardinalityMap : public MaxCardinalitySearch<
                                 Digraph,
                                 CapacityMap,
  ...
  /// for the algorithm.
  template<class T>
  struct SetProcessedMap : public MaxCardinalitySearch<
                               Digraph,
                               CapacityMap,
                               DefProcessedMapTraits<T>> {
    typedef MaxCardinalitySearch<Digraph, CapacityMap, DefProcessedMapTraits<T>>
        Create;
  };
  ...
  struct SetHeap : public MaxCardinalitySearch<
                       Digraph,
                       CapacityMap,
                       DefHeapTraits<H, CR>> {
    typedef MaxCardinalitySearch<Digraph, CapacityMap, DefHeapTraits<H, CR>>
        Create;
  };
  ...
  template<class H, class CR = typename Digraph::template NodeMap<int>>
  struct SetStandardHeap : public MaxCardinalitySearch<
                               Digraph,
                               CapacityMap,
 ...
 public:
  ...
  /// the algorithm.
  const CapacityMap& capacityMap() const { return *_capacity; }
  ...
  /// the algorithm.
  const ProcessedMap& processedMap() const { return *_processed; }
  ...
  /// of the heap.
  const HeapCrossRef& heapCrossRef() const { return *_heap_cross_ref; }
 ...
 public:
  ...
  Node nextNode() { return !_heap->empty() ? _heap->top() : INVALID; }
  ...
  void start(Node dest) {
    while (!_heap->empty() && _heap->top() != dest)
      processNextNode();
    if (!_heap->empty())
