...
struct PlanarityVisitor : DfsVisitor<Graph> {
  ...
  PlanarityVisitor(
      ...
      AncestorMap& ancestor_map,
      LowMap&      low_map)
      : _graph(graph),
        _pred_map(pred_map),
        _tree_map(tree_map),
        _order_map(order_map),
        _order_list(order_list),
        _ancestor_map(ancestor_map),
        _low_map(low_map) {}
...
class PlanarityChecking {
 ...
 public:
  ...
  bool run() {
    ...
    for (int i = order_list.size() - 1; i >= 0; --i) {
      ...
      for (typename MergeRoots::Value::iterator it = merge_roots[node].begin();
           it != merge_roots[node].end();
           ++it) {
        int rn = *it;
        walkDown(
            rn,
 ...
 private:
  void createChildLists(
      const TreeMap&  tree_map,
      const OrderMap& order_map,
      const LowMap&   low_map,
  ...
  void walkUp(
    ...
    while (true) {
      if (node_data[na].visited == rorder)
        break;
      if (node_data[nb].visited == rorder)
        break;
      ...
      while (n != rn) {
        ...
        if (embed_arc[node]) {
          ...
          while (!merge_stack.empty()) {
            ...
            if (cd)
              node_data[cn].next = dn;
            else
              node_data[cn].prev = dn;
            if (dd)
              node_data[dn].prev = cn;
            else
              node_data[dn].next = cn;
          }

          bool d = pn == node_data[n].prev;

          if (node_data[n].prev == node_data[n].next && node_data[n].inverted) {
            d = !d;
          }
        ...
        if (!merge_roots[node].empty()) {
          ...
          if (!external(xnode, rorder, child_lists, ancestor_map, low_map)) {
            rd = true;
                         ...
                         ynode, rorder, child_lists, ancestor_map, low_map)) {
            rd = false;
          ...
          n  = rd ? xn : yn;
                       ...
                       node, rorder, child_lists, ancestor_map, low_map)) {
          int nn =
              (node_data[n].next != pn ? node_data[n].next : node_data[n].prev);

          bool nd = n == node_data[nn].prev;

          if (nd)
            node_data[nn].prev = pn;
          else
            node_data[nn].next = pn;

          if (n == node_data[pn].prev)
            node_data[pn].prev = nn;
          else
            node_data[pn].next = nn;
  ...
  bool external(
    ...
    if (child != INVALID) {
      if (low_map[child] < rorder)
        return true;
    }

    if (ancestor_map[node] < rorder)
      return true;
...
class PlanarEmbedding {
 ...
 public:
  ...
  bool run(bool kuratowski = true) {
    ...
    for (int i = order_list.size() - 1; i >= 0; --i) {
      ...
      for (OutArcIt e(_graph, node); e != INVALID; ++e) {
        ...
        if (order_map[source] < order_map[target] && tree_map[e]) {
          initFace(
              target, arc_lists, node_data, pred_map, order_map, order_list);
        }
      }
    ...
    for (int i = 0; i < int(order_list.size()); ++i) {
      mergeRemainingFaces(
          ...
          arc_lists);
      storeEmbedding(
          order_list[i], node_data, order_map, pred_map, arc_lists, flip_map);
    }
  ...
  /// a node.
  Arc next(const Arc& arc) const { return _embedding[arc]; }
  ...
  const EmbeddingMap& embeddingMap() const { return _embedding; }
  ...
  bool kuratowski(const Edge& edge) const { return _kuratowski[edge]; }
 ...
 private:
  ...
  void storeEmbedding(
      ...
      ArcLists&   arc_lists,
      FlipMap&    flip_map) {
    if (node_data[order_map[node]].first == INVALID)
      return;
    ...
    Arc arc = flip_map[node] ? arc_lists[prev].prev : arc_lists[prev].next;

    _embedding[prev] = arc;

    while (arc != first) {
      Arc next = arc_lists[arc].prev == prev ? arc_lists[arc].next
                                             : arc_lists[arc].prev;
      prev             = arc;
      arc              = next;
      _embedding[prev] = arc;
  ...
  void orientComponent(
    ...
    while (!st.empty()) {
      ...
      while (arc != last) {
        ...
        Arc next = arc_lists[arc].next != pred ? arc_lists[arc].next
                                               : arc_lists[arc].prev;
        pred = arc;
        arc  = next;
      }
  ...
  int markExternalPath(
      ...
      AncestorMap& ancestor_map,
      LowMap&      low_map) {
    int lp = lowPoint(node, order_map, child_lists, ancestor_map, low_map);
  ...
  void isolateKuratowski(
    ...
    int rn = findComponentRoot(root, enode, child_lists, order_map, order_list);
    ...
    // Minor-A
    {
      ...
      if (root != _graph.source(arc)) {
        ...
        int xlp = markExternalPath(
            xnode, order_map, child_lists, pred_map, ancestor_map, low_map);
        int ylp = markExternalPath(
            ynode, order_map, child_lists, pred_map, ancestor_map, low_map);
        markPredPath(root, order_list[xlp < ylp ? xlp : ylp], pred_map);
        Node lwnode =
            findPertinent(ynode, order_map, node_data, embed_arc, merge_roots);
    ...
    if (!merge_roots[wnode].empty()) {
      ...
      if (low_map[rep] < rorder) {
        ...
        markPertinentPath(
            lwnode, order_map, node_data, arc_lists, embed_arc, merge_roots);
        int zlp = markExternalPath(
            lznode, order_map, child_lists, pred_map, ancestor_map, low_map);
    ...
    std::vector<Arc> ipath;
    findInternalPath(
        ipath, wnode, root, type_map, order_map, node_data, arc_lists);
    setInternalFlags(ipath, type_map);
    pynode = _graph.source(ipath.front());
    pxnode = _graph.target(ipath.back());

    wnode = findPertinent(pynode, order_map, node_data, embed_arc, merge_roots);
    ...
    // Minor-E*
    {
      if (!external(wnode, rorder, child_lists, ancestor_map, low_map)) {
        ...
        if (type_map[znode] == LOWY) {
          ...
          int xlp = markExternalPath(
              xnode, order_map, child_lists, pred_map, ancestor_map, low_map);
          int zlp = markExternalPath(
              znode, order_map, child_lists, pred_map, ancestor_map, low_map);
      ...
      int ylp = markExternalPath(
          ynode, order_map, child_lists, pred_map, ancestor_map, low_map);
      int wlp = markExternalPath(
          wnode, order_map, child_lists, pred_map, ancestor_map, low_map);
    ...
    while (e != l) {
      ...
      if (visited[graph.source(e)]) {
        typename Graph::Arc n =
            graph.direct(graph.addEdge(graph.source(p), graph.target(e)), true);
        embedding[n]                     = p;
        embedding[graph.oppositeArc(pp)] = n;

        embedding[graph.oppositeArc(n)] = embedding[graph.oppositeArc(e)];
        embedding[graph.oppositeArc(e)] = graph.oppositeArc(n);
...
class PlanarDrawing {
 public:
  ...
  /// \pre The graph must be simple, i.e. it should not
  /// contain parallel or loop arcs.
  PlanarDrawing(const Graph& graph) : _graph(graph), _point_map(graph) {}
 ...
 public:
  ...
  void run(const EmbeddingMap& embedding) {
    ...
    typename AuxGraph::template ArcMap<typename AuxGraph::Arc> aux_embedding(
        aux_graph);

    {
      typename Graph::template EdgeMap<typename AuxGraph::Edge> ref(_graph);
  ...
  Point operator[](const Node& node) const { return _point_map[node]; }
  ...
  const PointMap& coords() const { return _point_map; }
...
class PlanarColoring {
 public:
  ...
  IndexMap colorIndexMap() const { return _color_map; }
