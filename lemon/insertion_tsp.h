 ...
 public:
  ...
  Cost run(SelectionRule rule = FARTHEST) {
    ...
    switch (rule) {
      case NEAREST:
        init(true);
        start<ComparingSelection<std::less<Cost>>, DefaultInsertion>();
        break;
      case FARTHEST:
        init(false);
        start<ComparingSelection<std::greater<Cost>>, DefaultInsertion>();
        break;
      case CHEAPEST:
        init(true);
  ...
  ///
  /// \pre run() must be called before using this function.
  const std::vector<Node>& tourNodes() const { return _tour; }
 ...
 private:
  ...
  class CheapestSelection {
   private:
    Cost costDiff(Node u, Node v, Node w) const {
      return _cost[_gr.edge(u, w)] + _cost[_gr.edge(v, w)] -
             _cost[_gr.edge(u, v)];
    }
  ...
  class DefaultInsertion {
   ...
   public:
    ...
    void insert(Node n) const {
      int  min     = 0;
      Cost min_val = costDiff(_tour.front(), _tour.back(), n);
  ...
  class CheapestInsertion {
   ...
   public:
    ...
    void insert(Cost diff) const { _total += diff; }
