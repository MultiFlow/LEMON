...
class HowardMmc {
 ...
 public:
  ...
  template<typename T>
  struct SetLargeCost : public HowardMmc<GR, CM, SetLargeCostTraits<T>> {
  ...
  /// and it must have an \c addBack() function.
  template<typename T>
  struct SetPath : public HowardMmc<GR, CM, SetPathTraits<T>> {
 ...
 public:
  ...
  bool findCycle() {
    ...
    for (Node v = _best_node; (v = _gr.target(_policy[v])) != _best_node;) {
