...
class CapacityScaling {
  ...
  class ResidualDijkstra {
   ...
   public:
    ResidualDijkstra(CapacityScaling& cs)
        : _node_num(cs._node_num),
          _geq(cs._sum_supply < 0),
          _first_out(cs._first_out),
          _target(cs._target),
          _cost(cs._cost),
          _res_cap(cs._res_cap),
          _excess(cs._excess),
          _pi(cs._pi),
          _pred(cs._pred),
          _dist(cs._node_num) {}
 ...
 public:
  ...
  /// must be \ref RangeMap "RangeMap<int>".
  template<typename T>
  struct SetHeap : public CapacityScaling<GR, V, C, SetHeapTraits<T>> {
 ...
 private:
  ...
  ProblemType startWithScaling() {
    ...
    while (true) {
      ...
      for (int u = 0; u != _node_num; ++u) {
        last_out = _sum_supply < 0 ? _first_out[u + 1] : _first_out[u + 1] - 1;
        for (int a = _first_out[u]; a != last_out; ++a) {
          int   v  = _target[a];
      ...
      while (next_node < int(_excess_nodes.size())) {
        ...
        if (_delta > 1) {
          ...
          for (; next_def_node < int(_deficit_nodes.size()); ++next_def_node) {
  ...
  ProblemType startWithoutScaling() {
    ...
    for (int i = 0; i != _node_num; ++i) {
      if (_excess[i] > 0)
        _excess_nodes.push_back(i);
    }
    if (_excess_nodes.size() == 0)
      return OPTIMAL;
    int next_node = 0;
