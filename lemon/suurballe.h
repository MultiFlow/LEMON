...
#else
template<typename GR, typename LEN = typename GR::template ArcMap<int>>
#endif
...
class Suurballe {
 ...
 private:
  ...
  class ResidualDijkstra {
   ...
   public:
    ...
    bool run(int cnt) { return cnt == 0 ? startFirst() : start(); }
 ...
 public:
  ...
  /// \c FlowMap type.
  template<typename T>
  struct SetFlowMap : public Suurballe<GR, LEN, SetFlowMapTraits<T>> {
  ...
  /// \c PotentialMap type.
  template<typename T>
  struct SetPotentialMap : public Suurballe<GR, LEN, SetPotentialMapTraits<T>> {
  ...
  /// and it must have an \c addBack() function.
  template<typename T>
  struct SetPath : public Suurballe<GR, LEN, SetPathTraits<T>> {
  ...
  struct SetHeap : public Suurballe<GR, LEN, SetHeapTraits<H, CR>> {
 ...
 public:
  ...
  void fullInit(const Node& s) {
    ...
    // Run a full Dijkstra
    typename Dijkstra<Digraph, LengthMap>::template SetStandardHeap<Heap>::
        template SetDistMap<PotentialMap>::template SetPredMap<PredMap>::Create
            dijk(_graph, _length);
  ...
  /// \pre \ref run() or \ref findFlow() must be called before using
  /// this function.
  int flow(const Arc& arc) const { return (*_flow)[arc]; }
  ...
  /// \pre \ref run() or \ref findFlow() must be called before using
  /// this function.
  Length potential(const Node& node) const { return (*_potential)[node]; }
  ...
  /// \pre \ref run() or \ref findFlow() must be called before using
  /// this function.
  const PotentialMap& potentialMap() const { return *_potential; }
  ...
  /// \pre \ref run() or \ref findFlow() must be called before using
  /// this function.
  int pathNum() const { return _path_num; }
