    ...
    switch (c) {
      case 'p':
        if (is >> problem >> r.nodeNum >> r.edgeNum) {
          ...
          r.lineShift++;
          if (problem == "min")
            r.type = DimacsDescriptor::MIN;
          else if (problem == "max")
            r.type = DimacsDescriptor::MAX;
          else if (problem == "sp")
            r.type = DimacsDescriptor::SP;
          else if (problem == "mat")
            r.type = DimacsDescriptor::MAT;
          else
            throw FormatError("Unknown problem type");
          return r;
        } else {
...
void readDimacsMin(
  ...
  char                                c;
  int                                 i, j;
  if (desc.type == DimacsDescriptor::NONE)
    desc = dimacsType(is);
