...
void ArgParser::_terminate(ArgParserException::Reason reason) const {
  if (_exit_on_problems)
    ...
    throw(ArgParserException(reason));
}
...
ArgParser& ArgParser::doubleOption(
    const std::string& name,
    const std::string& help,
    double             value,
    bool               obl) {
...
ArgParser& ArgParser::boolOption(
    const std::string& name,
    const std::string& help,
    bool               value,
    bool               obl) {
...
ArgParser& ArgParser::stringOption(
    const std::string& name,
    const std::string& help,
    std::string        value,
    bool               obl) {
...
ArgParser& ArgParser::funcOption(
    const std::string& name,
    const std::string& help,
    void (*func)(void*),
    void* data) {
...
ArgParser& ArgParser::optionGroup(
    const std::string& group,
    const std::string& opt) {
  Opts::iterator i = _opts.find(opt);
  LEMON_ASSERT(i != _opts.end(), "Unknown option: '" + opt + "'");
  LEMON_ASSERT(
      !(i->second.ingroup), "Option already in option group: '" + opt + "'");
...
ArgParser& ArgParser::synonym(const std::string& syn, const std::string& opt) {
  Opts::iterator o = _opts.find(opt);
  LEMON_ASSERT(o != _opts.end(), "Unknown option: '" + opt + "'");
  LEMON_ASSERT(
      _opts.find(syn) == _opts.end(), "Option already used: '" + syn + "'");
  ParData p;
  p.help            = opt;
  p.mandatory       = false;
  p.syn             = true;
...
ArgParser& ArgParser::other(const std::string& name, const std::string& help) {
...
void ArgParser::show(std::ostream& os, Groups::const_iterator i) const {
  ...
  while (o != i->second.opts.end()) {
    ...
    ++o;
    if (o != i->second.opts.end())
      os << '|';
  }
}

void ArgParser::showHelp(Opts::const_iterator i) const {
  if (i->second.help.size() == 0 || i->second.syn)
    return;
  std::cerr << "  ";
  show(std::cerr, i);
  std::cerr << std::endl;
...
void ArgParser::showHelp(
    std::vector<ArgParser::OtherArg>::const_iterator i) const {
  if (i->help.size() == 0)
    return;
  std::cerr << "  " << i->name << std::endl << "     " << i->help << std::endl;
}
  ...
  for (int ar = 1; ar < _argc; ++ar) {
    ...
    if (arg[0] != '-' || arg.size() == 1) {
      ...
      if (i == _opts.end())
        ...
        if (i->second.syn)
          i = _opts.find(i->second.help);
        ParData& p(i->second);
        if (p.type == BOOL)
          *p.bool_p = true;
        else if (p.type == FUNC)
          p.func_p.p(p.func_p.data);
        else if (++ar == _argc)
