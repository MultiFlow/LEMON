...
template<
    ...
    typename TR = MinCostArborescenceDefaultTraits<GR, CM>>
#else
...
class MinCostArborescence {
 ...
 private:
  ...
  Arc prepare(Node node) {
    ...
    for (InArcIt it(*_digraph, node); it != INVALID; ++it) {
      ...
      if (source == node || (*_node_order)[source] == -3)
        continue;
    ...
    DualVariable var(
        _dual_node_list.size() - 1, _dual_node_list.size(), minimum.value);
  ...
  Arc contract(Node node) {
    ...
    while (!level_stack.empty() &&
      ...
      for (int i = 0; i < int(level_stack.back().arcs.size()); ++i) {
        ...
        if ((*_node_order)[source] >= node_bottom)
          continue;
        if ((*_cost_arcs)[source].arc == INVALID) {
          (*_cost_arcs)[source].arc   = arc;
          (*_cost_arcs)[source].value = value;
 ...
 public:
  ...
  template<class T>
  struct SetArborescenceMap : public MinCostArborescence<
                                  Digraph,
                                  CostMap,
  ...
  /// \brief Destructor.
  ~MinCostArborescence() { destroyStructures(); }
  ...
  bool emptyQueue() const { return queue.empty(); }
  ...
  /// \pre \ref run() must be called before using this function.
  const ArborescenceMap& arborescenceMap() const { return *_arborescence; }

  /// \brief Returns the predecessor arc of the given node.
  ///
  /// Returns the predecessor arc of the given node.
  /// \pre \ref run() must be called before using this function.
  Arc pred(Node node) const { return (*_pred)[node]; }
  ...
  bool reached(Node node) const { return (*_node_order)[node] != -3; }

  /// \brief Indicates that a node is processed.
  ///
  /// Indicates that a node is processed. The arborescence path exists
  /// from the source to the given node.
  bool processed(Node node) const { return (*_node_order)[node] == -1; }

  /// \brief Returns the number of the dual variables in basis.
  ///
  /// Returns the number of the dual variables in basis.
  int dualNum() const { return _dual_variables.size(); }
  ...
  Value dualValue(int k) const { return _dual_variables[k].value; }
  ...
  class DualIt {
   public:
    ...
    operator Node() const { return _algorithm->_dual_node_list[_index]; }
...
typename CostMap::Value minCostArborescence(
    const Digraph&         digraph,
    const CostMap&         cost,
    typename Digraph::Node source,
    ArborescenceMap&       arborescence) {
  typename MinCostArborescence<Digraph, CostMap>::template SetArborescenceMap<
      ArborescenceMap>::Create mca(digraph, cost);
