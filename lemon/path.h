...
class Path {
 public:
  ...
  ///
  Path(const Path& cpath) { pathCopy(cpath, *this); }
  ...
  class ArcIt {
   ...
   public:
    ...
    ArcIt(const Path& _path) : path(&_path), idx(_path.empty() ? -1 : 0) {}

   private:
    ArcIt(const Path& _path, int _idx) : path(&_path), idx(_idx) {}

   public:
    /// \brief Conversion to Arc
    operator const Arc&() const { return path->nth(idx); }
  ...
  ArcIt nthIt(int n) const { return ArcIt(*this, n); }
  ...
  /// it runs in O(1) time.
  /// \pre \c n is in the range <tt>[0..length() - 1]</tt>.
  const Arc& operator[](int n) const { return nth(n); }

  /// \brief The first arc of the path
  const Arc& front() const { return head.empty() ? tail.front() : head.back(); }

  /// \brief Add a new arc before the current path
  void addFront(const Arc& arc) { head.push_back(arc); }
  ...
  void eraseFront() {
    if (!head.empty()) {
      ...
      std::copy(tail.begin() + 1, tail.begin() + halfsize + 1, head.rbegin());
  ...
  const Arc& back() const { return tail.empty() ? head.front() : tail.back(); }

  /// \brief Add a new arc behind the current path
  void addBack(const Arc& arc) { tail.push_back(arc); }
  ...
  void eraseBack() {
    if (!tail.empty()) {
      ...
      std::copy(head.begin() + 1, head.begin() + halfsize + 1, tail.rbegin());
...
class SimplePath {
 public:
  ...
  /// \brief Copy constructor
  ///
  SimplePath(const SimplePath& cpath) { pathCopy(cpath, *this); }
  ...
  class ArcIt {
   ...
   private:
    ...
    ArcIt(const SimplePath& _path, int _idx) : path(&_path), idx(_idx) {}
  ...
  /// Gives back the n-th arc. This function runs in O(1) time.
  /// \pre \c n is in the range <tt>[0..length() - 1]</tt>.
  const Arc& nth(int n) const { return data[n]; }
  ...
  /// it runs in O(1) time.
  /// \pre \c n is in the range <tt>[0..length() - 1]</tt>.
  const Arc& operator[](int n) const { return data[n]; }

  /// \brief The first arc of the path.
  const Arc& front() const { return data.front(); }

  /// \brief The last arc of the path.
  const Arc& back() const { return data.back(); }

  /// \brief Add a new arc behind the current path.
  void addBack(const Arc& arc) { data.push_back(arc); }

  /// \brief Erase the last arc of the path
  void eraseBack() { data.pop_back(); }
 ...
 public:
  ...
  class ArcIt {
   ...
   public:
    ...
    /// \brief Initializate the constructor to the first arc of path
    ArcIt(const ListPath& _path) : path(&_path), node(_path.first) {}

   protected:
    ArcIt(const ListPath& _path, Node* _node) : path(&_path), node(_node) {}

   public:
    /// Conversion to Digraph::Arc
    operator const Arc&() const { return node->arc; }
  ...
  /// \brief The first arc of the path
  const Arc& front() const { return first->arc; }
  ...
  /// \brief The last arc of the path.
  const Arc& back() const { return last->arc; }
...
class StaticPath {
 public:
  ...
  /// \brief Copy constructor
  ///
  StaticPath(const StaticPath& cpath) : _arcs(0) { pathCopy(cpath, *this); }
  ...
  class ArcIt {
   ...
   private:
    /// Constructor with starting point
    ArcIt(const StaticPath& _path, int _idx) : idx(_idx), path(&_path) {}
  ...
  /// Gives back the n-th arc. This function runs in O(1) time.
  /// \pre \c n is in the range <tt>[0..length() - 1]</tt>.
  const Arc& nth(int n) const { return _arcs[n]; }
  ...
  /// it runs in O(1) time.
  /// \pre \c n is in the range <tt>[0..length() - 1]</tt>.
  const Arc& operator[](int n) const { return _arcs[n]; }
  ...
  /// \brief The first arc of the path.
  const Arc& front() const { return _arcs[0]; }

  /// \brief The last arc of the path.
  const Arc& back() const { return _arcs[_len - 1]; }
...
class PathNodeIt {
 ...
 public:
  ...
  PathNodeIt() {}
  /// Invalid constructor
  PathNodeIt(Invalid) : _digraph(0), _it(INVALID), _nd(INVALID) {}
  /// Constructor
  ...
  operator Node() const { return _nd; }
...
///\endcode
template<typename Path>
LemonRangeWrapper2<PathNodeIt<Path>, typename Path::Digraph, Path> pathNodes(
    const typename Path::Digraph& g,
    const Path&                   p) {
  return LemonRangeWrapper2<PathNodeIt<Path>, typename Path::Digraph, Path>(
      g, p);
}
