...
class RangeMap : public MapBase<int, V> {
 ...
 public:
  ...
  RangeMap(int size = 0, const Value& value = Value()) : _vector(size, value) {}
 ...
 public:
  ...
  ///\e
  Reference operator[](const Key& k) { return _vector[k]; }

  ///\e
  ConstReference operator[](const Key& k) const { return _vector[k]; }

  ///\e
  void set(const Key& k, const Value& v) { _vector[k] = v; }
};
...
/// \relates SparseMap
template<typename K, typename V, typename Compare>
inline SparseMap<K, V, Compare> sparseMap(
    const std::map<K, V, Compare>& map,
    const V&                       value = V()) {
...
class ComposeMap : public MapBase<typename M2::Key, typename M1::Value> {
 ...
 public:
  ...
  ///\e
  typename MapTraits<M1>::ConstReturnValue operator[](const Key& k) const {
    return _m1[_m2[k]];
  }
};
...
class CrossRefMap : protected ItemSetTraits<GR, K>::template Map<V>::Type {
 private:
  typedef typename ItemSetTraits<GR, K>::template Map<V>::Type Map;
 ...
 public:
  ...
  class ValueIt : public std::iterator<std::forward_iterator_tag, Value> {
   ...
   private:
    ValueIt(typename Container::const_iterator _it) : it(_it) {}
  ...
  /// map can be accessed in the <tt>[beginValue, endValue)</tt>
  /// range.
  ValueIt endValue() const { return ValueIt(_inv_map.end()); }
  ...
  void set(const Key& key, const Value& val) {
    ...
    for (it = _inv_map.equal_range(oldval).first;
         it != _inv_map.equal_range(oldval).second;
         ++it) {
      if (it->second == key) {
        _inv_map.erase(it);
        break;
  ...
  typename MapTraits<Map>::ConstReturnValue operator[](const Key& key) const {
  ...
  /// associated with it.
  int count(const Value& val) const { return _inv_map.count(val); }
 ...
 protected:
  ...
  virtual void erase(const Key& key) {
    ...
    for (it = _inv_map.equal_range(val).first;
         it != _inv_map.equal_range(val).second;
         ++it) {
      if (it->second == key) {
        _inv_map.erase(it);
        break;
 ...
 public:
  ...
  class InverseMap {
   public:
    ...
    explicit InverseMap(const CrossRefMap& inverted) : _inverted(inverted) {}
  ...
  InverseMap inverse() const { return InverseMap(*this); }
};
...
class IterableBoolMap
 ...
 public:
  ...
  LemonRangeWrapper1<TrueIt, IterableBoolMap> trueKeys() {
  ...
  /// It can be used in range-based for loops, STL algorithms, etc.
  LemonRangeWrapper1<FalseIt, IterableBoolMap> falseKeys() {
  ...
  class ItemIt : public Key {
   public:
    ...
    ItemIt(const IterableBoolMap& map, bool value)
        : Parent(
              value ? (map._sep > 0 ? map._array[map._sep - 1] : INVALID)
                    : (map._sep < int(map._array.size()) ? map._array.back()
                                                         : INVALID)),
          _map(&map) {}
  ...
  /// It can be used in range-based for loops, STL algorithms, etc.
  LemonRangeWrapper2<ItemIt, IterableBoolMap, bool> items(bool value) {
...
/// \see CrossRefMap
template<typename GR, typename K>
class IterableIntMap : protected ItemSetTraits<GR, K>::template Map<
                           _maps_bits::IterableIntMapNode<K>>::Type {
 public:
  typedef typename ItemSetTraits<GR, K>::template Map<
      _maps_bits::IterableIntMapNode<K>>::Type Parent;
 ...
 public:
  ...
  class ItemIt : public Key {
   public:
    ...
    ItemIt& operator++() {
      Parent::operator=(
          _map->IterableIntMap::Parent::operator[](static_cast<Parent&>(*this))
              .next);
      return *this;
    }
  ...
  /// This is an STL style wrapper for \ref ItemIt.
  /// It can be used in range-based for loops, STL algorithms, etc.
  LemonRangeWrapper2<ItemIt, IterableIntMap, int> items(int value) {
...
/// \see CrossRefMap
template<typename GR, typename K, typename V>
class IterableValueMap : protected ItemSetTraits<GR, K>::template Map<
                             _maps_bits::IterableValueMapNode<K, V>>::Type {
 public:
  typedef typename ItemSetTraits<GR, K>::template Map<
      _maps_bits::IterableValueMapNode<K, V>>::Type Parent;
 ...
 public:
  ...
  class ItemIt : public Key {
   public:
    ...
    /// Increment Operator.
    ItemIt& operator++() {
      Parent::operator=(_map->IterableValueMap::Parent::operator[](
  ...
  /// This is an STL style wrapper for \ref ItemIt.
  /// It can be used in range-based for loops, STL algorithms, etc.
  LemonRangeWrapper2<ItemIt, IterableValueMap, V> items(const V& value) {
...
/// \sa OutDegMap
template<typename GR>
class InDegMap : protected ItemSetTraits<GR, typename GR::Arc>::ItemNotifier::
                     ObserverBase {
 public:
  ...
  typedef typename ItemSetTraits<Digraph, typename Digraph::Arc>::ItemNotifier::
      ObserverBase Parent;
 ...
 private:
  class AutoNodeMap
   ...
   public:
    typedef
        typename ItemSetTraits<Digraph, Key>::template Map<int>::Type Parent;
 ...
 protected:
  ...
  virtual void add(const Arc& arc) { ++_deg[_digraph.target(arc)]; }
  ...
  virtual void erase(const Arc& arc) { --_deg[_digraph.target(arc)]; }
...
/// \sa InDegMap
template<typename GR>
class OutDegMap : protected ItemSetTraits<GR, typename GR::Arc>::ItemNotifier::
                      ObserverBase {
 public:
 ...
 protected:
  ...
  virtual void add(const Arc& arc) { ++_deg[_digraph.source(arc)]; }
  ...
  virtual void erase(const Arc& arc) { --_deg[_digraph.source(arc)]; }
...
class PotentialDifferenceMap {
 public:
  ...
  explicit PotentialDifferenceMap(const GR& gr, const POT& potential)
...
template<typename GR, typename POT>
PotentialDifferenceMap<GR, POT> potentialDifferenceMap(
    const GR&  gr,
    const POT& potential) {
