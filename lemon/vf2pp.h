...
class Vf2pp {
  ...
  typename G1::template NodeMap<std::vector<std::pair<int, int>>> _rNewLabels1;
  ...
  void processBfsTree(
    ...
    unsigned int endPosOfLevel = orderIndex, startPosOfLevel = orderIndex,
        ...
        for (typename G1::NodeIt n1(_g1, minNode); n1 != INVALID; ++n1)
          if (!added[n1] &&
              (_labelTmp1[_intLabels1[minNode]] > _labelTmp1[_intLabels1[n1]] ||
               (dm1[minNode] < dm1[n1] && _labelTmp1[_intLabels1[minNode]] ==
    ...
    while (_depth >= 0) {
      ...
      if (edgeItOfDepth == INVALID) {
        ...
        if (pairOfNodeOfDepth == INVALID) {
          for (;
               fstMatchedE != INVALID &&
               _mapping[_g1.oppositeNode(nodeOfDepth, fstMatchedE)] == INVALID;
               ++fstMatchedE)
    ...
    for (unsigned int i = 0; i < _order.size(); ++i) {
      ...
      for (typename G1::IncEdgeIt e1(_g1, _order[i]); e1 != INVALID; ++e1) {
        ...
        if (_labelTmp1[currIntLabel] > 0) {
          _rInOutLabels1[_order[i]].push_back(
              std::make_pair(currIntLabel, _labelTmp1[currIntLabel]));
          _labelTmp1[currIntLabel] = 0;
        } else if (_labelTmp2[currIntLabel] > 0) {
          _rNewLabels1[_order[i]].push_back(
              std::make_pair(currIntLabel, _labelTmp2[currIntLabel]));
 ...
 public:
  ...
  Vf2pp(const G1& g1, const G2& g2, M& m, M1& intLabels1, M2& intLabels2)
      : _g1(g1),
        ...
        _depth(0),
        _mapping(m),
        _order(countNodes(g1), INVALID),
        _conn(g2, 0),
        _currEdgeIts(countNodes(g1), INVALID),
        _rNewLabels1(_g1),
        _rInOutLabels1(_g1),
        _intLabels1(intLabels1),
        _intLabels2(intLabels2),
        _maxLabel(getMaxLabel()),
        _labelTmp1(_maxLabel + 1),
        _labelTmp2(_maxLabel + 1),
        _mapping_type(SUBGRAPH),
        _deallocMappingAfterUse(0),
...
class Vf2ppWizardBase {
 protected:
  ...
  bool                                                     _local_mapping;
  void*                                                    _mapping;
  void createMapping() { _mapping = new Mapping(_g1); }
...
class Vf2ppWizard : public TR {
 ...
 public:
  ...
  /// must be the numbers {0,1,..,K-1}.
  template<typename NL1, typename NL2>
  Vf2ppWizard<SetNodeLabelsBase<NL1, NL2>> nodeLabels(
      const NL1& nodeLabels1,
      const NL2& nodeLabels2) {
  ...
  bool run() {
    ...
    Vf2pp<Graph1, Graph2, Mapping, NodeLabels1, NodeLabels2> alg(
        _g1,
        _g2,
