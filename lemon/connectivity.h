...
int countConnectedComponents(const Graph& graph) {
  ...
  int compNum = 0;
  typename Bfs<Graph>::template SetPredMap<PredMap>::template SetDistMap<
      DistMap>::Create bfs(graph);
...
class BiNodeConnectedCutNodesVisitor : public DfsVisitor<Digraph> {
 public:
  ...
  BiNodeConnectedCutNodesVisitor(
      const Digraph& graph,
      NodeMap&       cutMap,
      int&           cutNum)
      : _graph(graph),
...
class BiEdgeConnectedComponentsVisitor : public DfsVisitor<Digraph> {
 public:
  ...
  BiEdgeConnectedComponentsVisitor(
      const Digraph& graph,
      NodeMap&       compMap,
      int&           compNum)
      : _graph(graph),
...
class BiEdgeConnectedCutEdgesVisitor : public DfsVisitor<Digraph> {
 public:
  ...
  BiEdgeConnectedCutEdgesVisitor(
      const Digraph& graph,
      ArcMap&        cutMap,
      int&           cutNum)
      : _graph(graph),
...
bool dag(const Digraph& digraph) {
  ...
  typename Dfs<Digraph>::template SetProcessedMap<ProcessedMap>::Create dfs(
      digraph);
...
void topologicalSort(const Digraph& digraph, NodeMap& order) {
  ...
  TopologicalSortVisitor<Digraph, NodeMap> visitor(order, countNodes(digraph));

  DfsVisit<Digraph, TopologicalSortVisitor<Digraph, NodeMap>> dfs(
      digraph, visitor);
...
bool tree(const Graph& graph) {
  ...
  while (!dfs.emptyQueue()) {
    ...
    Node source = graph.source(arc);
    Node target = graph.target(arc);
    if (dfs.reached(target) && dfs.predArc(source) != graph.oppositeArc(arc)) {
      return false;
    }
    dfs.processNextArc();
  }
...
class BipartiteVisitor : public BfsVisitor<Digraph> {
 public:
  ...
  void examine(const Arc& edge) {
    _bipartite =
        _bipartite && _part[_graph.target(edge)] != _part[_graph.source(edge)];
  }
...
bool bipartitePartitions(const Graph& graph, NodeMap& partMap) {
  ...
  BfsVisit<Graph, BipartitePartitionsVisitor<Graph, NodeMap>> bfs(
      graph, visitor);
  bfs.init();
  for (NodeIt it(graph); it != INVALID; ++it) {
    if (!bfs.reached(it)) {
...
bool loopFree(const Graph& graph) {
  for (typename Graph::ArcIt it(graph); it != INVALID; ++it) {
    if (graph.source(it) == graph.target(it))
      return false;
  }
  return true;
}
