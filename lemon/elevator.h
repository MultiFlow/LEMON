...
class Elevator {
 ...
 private:
  ...
  void copy(Item i, Vit p) { _where[*p = i] = p; }
 ...
 public:
  ...
  bool emptyLevel(int l) const { return _first[l + 1] - _first[l] == 0; }
  ...
  bool activeFree(int l) const { return _last_active[l] < _first[l]; }
  ...
  /// item.
  int highestActiveLevel() const { return _highest_active; }
  ...
  void dirtyTopButOne(Item i) { _level[i] = _max_level - 1; }
...
class LinkedElevator {
 ...
 private:
  typedef typename ItemSetTraits<GR, Item>::template Map<Item>::Type ItemMap;
  typedef typename ItemSetTraits<GR, Item>::template Map<int>::Type  IntMap;
  typedef typename ItemSetTraits<GR, Item>::template Map<bool>::Type BoolMap;
 ...
 public:
  ...
  void activate(Item i) {
    ...
    if (_prev[i] == INVALID || _active[_prev[i]])
      return;
    // unlace
  ...
  bool emptyLevel(int l) const { return _first[l] == INVALID; }
 ...
 public:
  ...
  void initStart() {
    ...
    _init_level = 0;
    for (typename ItemSetTraits<GR, Item>::ItemIt i(_graph); i != INVALID;
         ++i) {
      _level[i]  = _max_level;
      _active[i] = false;
    }
  ...
  /// Start a new level.
  /// It shouldn't be used before the items on level 0 are listed.
  void initNewLevel() { ++_init_level; }

  /// Finalize the initialization process.
  void initFinish() { _highest_active = -1; }
