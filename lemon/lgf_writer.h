...
class GraphArcMapLess {
 ...
 public:
  ...
  bool operator()(const Item& left, const Item& right) {
    return _map[_graph.direct(left, _dir)] < _map[_graph.direct(right, _dir)];
  }
};
...
class MapStorage : public MapStorageBase<_Item> {
 ...
 public:
  ...
  virtual std::string get(const Item& item) { return _converter(_map[item]); }
...
class ValueStorage : public ValueStorageBase {
 ...
 public:
  ...
  virtual std::string get() { return _converter(_value); }
};

template<typename Value, typename Map = std::map<Value, std::string>>
...
struct GraphArcLookUpConverter {
  ...
  GraphArcLookUpConverter(
      const Graph&                                       graph,
      const std::map<typename Graph::Edge, std::string>& map)
      : _graph(graph), _map(map) {}

  std::string operator()(const typename Graph::Arc& val) {
    typename std::map<typename Graph::Edge, std::string>::const_iterator it =
        _map.find(val);
    if (it == _map.end()) {
      throw FormatError("Item not found");
    }
...
class StreamSection : public Section {
 ...
 public:
  ...
  virtual void process(std::ostream& os) { _functor(os); }
};
...
DigraphWriter<TDGR> digraphWriter(
    const TDGR&   digraph,
...
class DigraphWriter {
 ...
 private:
  ...
  typedef std::vector<
      std::pair<std::string, _writer_bits::MapStorageBase<Node>*>>
           NodeMaps;
  NodeMaps _node_maps;

  typedef std::vector<
      std::pair<std::string, _writer_bits::MapStorageBase<Arc>*>>
          ArcMaps;
  ArcMaps _arc_maps;

  typedef std::vector<std::pair<std::string, _writer_bits::ValueStorageBase*>>
             Attributes;
 ...
 private:
  template<typename TDGR>
  friend DigraphWriter<TDGR> digraphWriter(
      const TDGR&   digraph,
      std::ostream& os);
  template<typename TDGR>
  friend DigraphWriter<TDGR> digraphWriter(
      ...
      const std::string& fn);
  template<typename TDGR>
  friend DigraphWriter<TDGR> digraphWriter(const TDGR& digraph, const char* fn);
  ...
  void writeArcs() {
    ...
    *_os << std::endl;
    ...
    for (int i = 0; i < static_cast<int>(arcs.size()); ++i) {
      Arc a = arcs[i];
      _writer_bits::writeToken(
          *_os, _node_index.find(_digraph.source(a))->second);
      *_os << '\t';
      _writer_bits::writeToken(
          *_os, _node_index.find(_digraph.target(a))->second);
      *_os << '\t';
      if (label == 0) {
        std::ostringstream os;
        os << _digraph.id(a);
...
class GraphWriter {
 ...
 private:
  ...
  typedef std::vector<
      std::pair<std::string, _writer_bits::MapStorageBase<Edge>*>>
           EdgeMaps;
  EdgeMaps _edge_maps;
 ...
 private:
  ...
  template<typename TGR>
  friend GraphWriter<TGR> graphWriter(const TGR& graph, const std::string& fn);
  template<typename TGR>
  ...
  void writeEdges() {
    ...
    *_os << std::endl;
    ...
    for (int i = 0; i < static_cast<int>(edges.size()); ++i) {
      Edge e = edges[i];
      _writer_bits::writeToken(*_os, _node_index.find(_graph.u(e))->second);
      *_os << '\t';
      _writer_bits::writeToken(*_os, _node_index.find(_graph.v(e))->second);
      *_os << '\t';
      if (label == 0) {
        std::ostringstream os;
        os << _graph.id(e);
...
BpGraphWriter<TBGR> bpGraphWriter(
    const TBGR&   graph,
    std::ostream& os = std::cout);
template<typename TBGR>
...
class BpGraphWriter {
 ...
 private:
  ...
  typedef std::vector<
      std::pair<std::string, _writer_bits::MapStorageBase<RedNode>*>>
              RedNodeMaps;
  RedNodeMaps _red_node_maps;
  typedef std::vector<
      std::pair<std::string, _writer_bits::MapStorageBase<BlueNode>*>>
               BlueNodeMaps;
 ...
 private:
  template<typename TBGR>
  friend BpGraphWriter<TBGR> bpGraphWriter(const TBGR& graph, std::ostream& os);
  template<typename TBGR>
  friend BpGraphWriter<TBGR> bpGraphWriter(
      const TBGR&        graph,
      const std::string& fn);
  template<typename TBGR>
  ...
  void writeEdges() {
    ...
    *_os << std::endl;
    ...
    for (int i = 0; i < static_cast<int>(edges.size()); ++i) {
      Edge e = edges[i];
      _writer_bits::writeToken(
          *_os, _red_node_index.find(_graph.redNode(e))->second);
      *_os << '\t';
      _writer_bits::writeToken(
          *_os, _blue_node_index.find(_graph.blueNode(e))->second);
      *_os << '\t';
      if (label == 0) {
        std::ostringstream os;
        os << _graph.id(e);
...
class SectionWriter {
 private:
  ...
  typedef std::vector<std::pair<std::string, _writer_bits::Section*>> Sections;
 ...
 public:
  ...
  /// stream.
  SectionWriter(std::ostream& os) : _os(&os), local_os(false) {}
  ...
  ///
  /// Construct a section writer, which writes into the given file.
  SectionWriter(const char* fn) : _os(new std::ofstream(fn)), local_os(true) {
    if (!(*_os)) {
      delete _os;
      throw IoError("Cannot write file", fn);
    }
 ...
 public:
  ...
  SectionWriter& sectionLines(const std::string& type, Functor functor) {
    ...
    _sections.push_back(
        std::make_pair(type, new _writer_bits::LineSection<Functor>(functor)));
    return *this;
  }
  ...
  SectionWriter& sectionStream(const std::string& type, Functor functor) {
    LEMON_ASSERT(!type.empty(), "Type is empty.");
    _sections.push_back(std::make_pair(
        type, new _writer_bits::StreamSection<Functor>(functor)));
    return *this;
  }
