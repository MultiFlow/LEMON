...
class DiEulerIt {
 ...
 public:
  ...
  DiEulerIt(const GR& gr, typename GR::Node start = INVALID) : g(gr), narc(g) {
    if (start == INVALID) {
      NodeIt n(g);
      while (n != INVALID && OutArcIt(g, n) == INVALID)
        ++n;
      start = n;
    }
    if (start != INVALID) {
...
class EulerIt {
 ...
 public:
  ...
  EulerIt(const GR& gr, typename GR::Node start = INVALID)
    ...
    if (start != INVALID) {
      ...
      while (narc[start] != INVALID) {
        ...
        ++narc[start];
        start = next;
        while (narc[start] != INVALID && visited[narc[start]])
          ++narc[start];
      }
    }
  }
  ...
  EulerIt& operator++() {
    ...
    typename std::list<Arc>::iterator next = euler.begin();
    while (narc[s] != INVALID) {
      while (narc[s] != INVALID && visited[narc[s]])
        ++narc[s];
      if (narc[s] == INVALID)
        break;
      else {
...
template<typename GR>
  ...
  bool
#else
typename enable_if<UndirectedTagIndicator<GR>, bool>::type eulerian(
    const GR& g) {
