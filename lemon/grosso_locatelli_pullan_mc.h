...
class GrossoLocatelliPullanMc {
  ...
  class DegreeBasedSelectionRule {
   ...
   public:
    ...
    int nextFeasibleSwapNode() const {
      ...
      int node = -1, max_deg = -1;
      for (int i = start_node; i != _n; i++) {
        if (!_clique[i] && _delta[i] == 1 && !_tabu[i] && _deg[i] > max_deg) {
          node    = i;
          max_deg = _deg[i];
        }
      }
  ...
  class PenaltyBasedSelectionRule {
   ...
   public:
    ...
    int nextFeasibleSwapNode() const {
      ...
      int node = -1, min_p = std::numeric_limits<int>::max();
      for (int i = start_node; i != _n; i++) {
        if (!_clique[i] && _delta[i] == 1 && !_tabu[i] && _penalty[i] < min_p) {
          node  = i;
          min_p = _penalty[i];
        }
      }
 ...
 public:
  ...
  ///
  /// \sa iterationLimit(int)
  int iterationLimit() const { return _iteration_limit; }
  ...
  ///
  /// \sa stepLimit(int)
  int stepLimit() const { return _step_limit; }
  ...
  ///
  /// \sa sizeLimit(int)
  int sizeLimit() const { return _size_limit; }
  ...
  int cliqueSize() const { return _best_size; }
  ...
  class CliqueNodeIt {
   ...
   public:
    ...
    CliqueNodeIt(const GrossoLocatelliPullanMc& mc) : _map(mc._graph) {
      mc.cliqueMap(_map);
 ...
 private:
  ...
  TerminationCause start() {
    ...
    const int max_select =
        _step_limit >= 0 ? _step_limit : std::numeric_limits<int>::max();
