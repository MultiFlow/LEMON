...
class BfsLeaveOrder : public BfsVisitor<G> {
 ...
 public:
  ...
  void process(const typename G::Node& node) { _order[i++] = node; }
};
} // namespace vf2
} // namespace bits
...
class Vf2Wizard : public TR {
 ...
 public:
  ...
  template<class M1, class M2>
  Vf2Wizard<SetNodeEqBase<bits::vf2::MapEq<M1, M2>>> nodeLabels(
      const M1& m1,
      const M2& m2) {
  ...
  bool run() {
    ...
    Vf2<Graph1, Graph2, Mapping, NodeEq> alg(
