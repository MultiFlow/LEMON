...
class ValueStorage : public ValueStorageBase {
 ...
 public:
  ...
  virtual void set(const std::string& value) { _value = _converter(value); }
};

template<typename Value, typename Map = std::map<std::string, Value>>
struct MapLookUpConverter {
  const Map& _map;

  MapLookUpConverter(const Map& map) : _map(map) {}
...
struct GraphArcLookUpConverter {
  ...
  GraphArcLookUpConverter(
      const GR&                                       graph,
      const std::map<std::string, typename GR::Edge>& map)
  ...
  typename GR::Arc operator()(const std::string& str) {
    ...
    typename std::map<std::string, typename GR::Edge>::const_iterator it =
        _map.find(str.substr(1));
    if (it == _map.end()) {
...
inline char readEscape(std::istream& is) {
  ...
  switch (c) {
    ...
    case 'x': {
      ...
      if (!is.get(c) || !isHex(c))
        throw FormatError("Escape format error");
      else if (code = valueHex(c), !is.get(c) || !isHex(c))
        is.putback(c);
      else
        code = code * 16 + valueHex(c);
      return code;
    }
    default: {
      int code;
...
class DigraphReader {
 ...
 private:
  ...
  typedef std::vector<
      std::pair<std::string, _reader_bits::MapStorageBase<Node>*>>
           NodeMaps;
  NodeMaps _node_maps;

  typedef std::vector<
      std::pair<std::string, _reader_bits::MapStorageBase<Arc>*>>
          ArcMaps;
  ArcMaps _arc_maps;
 ...
 private:
  ...
  template<typename TDGR>
  friend DigraphReader<TDGR> digraphReader(
      TDGR&              digraph,
      const std::string& fn);
  template<typename TDGR>
  ...
  DigraphReader(DigraphReader& other)
      : _is(other._is),
        local_is(other.local_is),
        _digraph(other._digraph),
        _use_nodes(other._use_nodes),
        _use_arcs(other._use_arcs),
        _skip_nodes(other._skip_nodes),
        _skip_arcs(other._skip_arcs) {
    other._is      = 0;
 ...
 private:
  ...
  bool readSuccess() { return static_cast<bool>(*_is); }
  ...
  void readArcs() {
...

    {
      ...
      for (int i = 0; i < static_cast<int>(_arc_maps.size()); ++i) {
        std::map<std::string, int>::iterator jt = maps.find(_arc_maps[i].first);
        if (jt == maps.end()) {
          std::ostringstream msg;
  ...
  void run() {
    ...
    while (readSuccess()) {
      try {
        ...
        } else if ((section == "arcs" || section == "edges") && !arcs_done) {
...
class GraphReader {
 ...
 private:
  ...
  typedef std::vector<
      std::pair<std::string, _reader_bits::MapStorageBase<Edge>*>>
           EdgeMaps;
  EdgeMaps _edge_maps;
 ...
 private:
  ...
  GraphReader(GraphReader& other)
      : _is(other._is),
        ...
        _graph(other._graph),
        _use_nodes(other._use_nodes),
        _use_edges(other._use_edges),
        _skip_nodes(other._skip_nodes),
        _skip_edges(other._skip_edges) {
    other._is      = 0;
    other.local_is = false;
  ...
  void run() {
    ...
    while (readSuccess()) {
      try {
        ...
        } else if ((section == "edges" || section == "arcs") && !edges_done) {
...
class BpGraphReader {
 ...
 private:
  ...
  typedef std::vector<
      std::pair<std::string, _reader_bits::MapStorageBase<RedNode>*>>
              RedNodeMaps;
  RedNodeMaps _red_node_maps;
  typedef std::vector<
      std::pair<std::string, _reader_bits::MapStorageBase<BlueNode>*>>
               BlueNodeMaps;
 ...
 private:
  ...
  template<typename TBGR>
  friend BpGraphReader<TBGR> bpGraphReader(TBGR& graph, const std::string& fn);
  template<typename TBGR>
 ...
 public:
  ...
  BpGraphReader& node(const std::string& caption, Node& node) {
    typedef _reader_bits::
        DoubleMapLookUpConverter<Node, RedNodeIndex, BlueNodeIndex>
              Converter;
...
class SectionReader {
 ...
 public:
  ...
  /// stream.
  SectionReader(std::istream& is) : _is(&is), local_is(false) {}

  /// \brief Constructor
  ///
  /// Construct a section reader, which reads from the given file.
  SectionReader(const std::string& fn)
      : _is(new std::ifstream(fn.c_str())), local_is(true), _filename(fn) {
    if (!(*_is)) {
      delete _is;
      throw IoError("Cannot open file", fn);
    }
  ...
  SectionReader(const char* fn)
      : _is(new std::ifstream(fn)), local_is(true), _filename(fn) {
    if (!(*_is)) {
      delete _is;
      throw IoError("Cannot open file", fn);
    }
  ...
  /// \brief Destructor
  ~SectionReader() {
    for (Sections::iterator it = _sections.begin(); it != _sections.end();
         ++it) {
      delete it->second;
    }
 ...
 public:
  ...
  SectionReader& sectionLines(const std::string& type, Functor functor) {
    ...
    LEMON_ASSERT(
        _sections.find(type) == _sections.end(),
        "Multiple reading of section.");
    _sections.insert(
        std::make_pair(type, new _reader_bits::LineSection<Functor>(functor)));
    return *this;
  }
  ...
  SectionReader& sectionStream(const std::string& type, Functor functor) {
    ...
    LEMON_ASSERT(
        _sections.find(type) == _sections.end(),
        "Multiple reading of section.");
    _sections.insert(std::make_pair(
        type, new _reader_bits::StreamSection<Functor>(functor)));
    return *this;
  }
...
class LgfContents {
 ...
 public:
  ...
  /// input stream.
  LgfContents(std::istream& is) : _is(&is), local_is(false) {}
  ...
  /// Construct an \e LGF contents reader, which reads from the given
  /// file.
  LgfContents(const char* fn) : _is(new std::ifstream(fn)), local_is(true) {
    if (!(*_is)) {
      delete _is;
      throw IoError("Cannot open file", fn);
    }
 ...
 public:
  ...
  int nodeSectionNum() const { return _node_sections.size(); }

  /// \brief Returns the node section name at the given position.
  ///
  /// Returns the node section name at the given position.
  const std::string& nodeSection(int i) const { return _node_sections[i]; }
  ...
  int arcSectionNum() const { return _edge_sections.size(); }

  /// \brief Returns the arc/edge section name at the given position.
  ///
  /// Returns the arc/edge section name at the given position.
  /// \note It is synonym of \c edgeSection().
  const std::string& arcSection(int i) const { return _edge_sections[i]; }
  ...
  int edgeSectionNum() const { return _edge_sections.size(); }

  /// \brief Returns the section name at the given position.
  ///
  /// Returns the section name at the given position.
  /// \note It is synonym of \c arcSection().
  const std::string& edgeSection(int i) const { return _edge_sections[i]; }
  ...
  int attributeSectionNum() const { return _attribute_sections.size(); }
  ...
  int extraSectionNum() const { return _extra_sections.size(); }

  /// \brief Returns the extra section type at the given position.
  ///
  /// Returns the section type at the given position.
  const std::string& extraSection(int i) const { return _extra_sections[i]; }
