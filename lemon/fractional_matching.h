...
#else
template<typename GR, typename TR = MaxFractionalMatchingDefaultTraits<GR>>
#endif
class MaxFractionalMatching {
 ...
 public:
  ...
  struct SetStandardElevator
    ...
    typedef MaxFractionalMatching<Graph, SetStandardElevatorTraits<T>> Create;
  };
 ...
 public:
  ...
  ~MaxFractionalMatching() { destroyStructures(); }
  ...
  /// using this function.
  const Elevator& elevator() const { return *_level; }
  ...
  void init() {
    ...
    for (NodeIt n(_graph); n != INVALID; ++n) {
      for (OutArcIt a(_graph, n); a != INVALID; ++a) {
        if (_graph.target(a) == n && !_allow_loops)
          continue;
        _matching->set(n, a);
        Node v = _graph.target((*_matching)[n]);
      ...
      if (node == v) {
        ...
        if ((*_pred)[v] == a) {
          ...
          for (OutArcIt aa(_graph, v); aa != INVALID; ++aa) {
            ...
            if ((*_status)[va] != EVEN || _tree_set->find(va) == tree)
              continue;
  ...
  void init() {
    ...
    for (NodeIt n(_graph); n != INVALID; ++n) {
      ...
      for (OutArcIt e(_graph, n); e != INVALID; ++e) {
        if (_graph.target(e) == n && !_allow_loops)
          continue;
    ...
    for (EdgeIt e(_graph); e != INVALID; ++e) {
      ...
      if (left == right && !_allow_loops)
        continue;
      _delta3->push(
          e,
          ((*_node_potential)[left] + (*_node_potential)[right] -
           dualScale * _weight[e]) /
              2);
    }
  }
      ...
      switch (ot) {
        ...
        case D2: {
          ...
          if ((*_matching)[n] == INVALID) {
            ...
            Node v = _graph.target((*_matching)[n]);
            if ((*_matching)[n] != _graph.oppositeArc((*_matching)[v])) {
              extractCycle(a);
              --unmatched;
            } else {
              extendOnArc(a);
