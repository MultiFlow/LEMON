...
Iterator
radixSortPartition(Iterator first, Iterator last, Functor functor, Value mask) {
...
Iterator
radixSortSignPartition(Iterator first, Iterator last, Functor functor) {
...
void stableRadixIntroSort(
    Key*    first,
    Key*    last,
    Key*    target,
    int     byte,
    Functor functor) {
  const int size = unsigned(std::numeric_limits<unsigned char>::max()) + 1;
...
template<typename Functor, typename Key>
void signedStableRadixIntroSort(
    Key*    first,
    Key*    last,
    Key*    target,
    int     byte,
...
void stableRadixSignedSort(Iterator first, Iterator last, Functor functor) {
  ...
  try {
    ...
    for (int i = 0; i < int(sizeof(Value)) - 1; ++i) {
      if (dir) {
        stableRadixIntroSort(
            buffer, buffer + length, buffer + length, i, functor);
      } else {
        stableRadixIntroSort(
            buffer + length, buffer + 2 * length, buffer, i, functor);
      }
      dir = !dir;
    }
    if (dir) {
      signedStableRadixIntroSort(
          buffer, buffer + length, buffer + length, sizeof(Value) - 1, functor);
...
template<typename Value, typename Iterator, typename Functor>
void stableRadixUnsignedSort(Iterator first, Iterator last, Functor functor) {
  if (first == last)
    return;
  typedef typename std::iterator_traits<Iterator>::value_type Key;
  typedef std::allocator<Key>                                 Allocator;
