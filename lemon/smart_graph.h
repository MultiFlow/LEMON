...
class SmartDigraphBase {
 ...
 public:
  ...
  void first(Node& node) const { node._id = _nodes.size() - 1; }
 ...
 public:
  ...
  class Snapshot {
   ...
   public:
    ...
    void restore() { _graph->restoreSnapshot(*this); }
  };
};
...
class SmartGraphBase {
 ...
 public:
  ...
  static bool direction(Arc e) { return (e._id & 1) == 1; }
  ...
  void first(Edge& arc) const { arc._id = _arcs.size() / 2 - 1; }
 ...
 public:
  ...
  class Snapshot {
   ...
   public:
    ...
    ///
    Snapshot(SmartGraph& gr) { gr.saveSnapshot(*this); }
    ...
    /// It can be called more than once. In case of a repeated
    /// call, the previous snapshot gets lost.
    void save(SmartGraph& gr) { gr.saveSnapshot(*this); }
...
class SmartBpGraphBase {
 ...
 public:
  ...
  RedNode  redNode(Edge e) const { return RedNode(_arcs[2 * e._id].target); }
  ...
  void first(RedNode& node) const { node._id = first_red; }

  void next(RedNode& node) const { node._id = _nodes[node._id].partition_next; }

  void first(BlueNode& node) const { node._id = first_blue; }
 ...
 public:
  ...
  class Snapshot {
   ...
   public:
    ...
    /// This constructor immediately makes a snapshot of the given graph.
    ///
    Snapshot(SmartBpGraph& gr) { gr.saveSnapshot(*this); }
    ...
    /// It can be called more than once. In case of a repeated
    /// call, the previous snapshot gets lost.
    void save(SmartBpGraph& gr) { gr.saveSnapshot(*this); }
