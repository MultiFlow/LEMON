...
class VarIndex {
 ...
 public:
  VarIndex() : first_item(-1), last_item(-1), first_free_item(-1) {}
  ...
  int maxIndex() const { return cross.size() - 1; }
  ...
  int operator[](int idx) const { return cross[idx]; }

  int operator()(int fdx) const { return items[fdx].index; }

  void firstItem(int& fdx) const { fdx = first_item; }
