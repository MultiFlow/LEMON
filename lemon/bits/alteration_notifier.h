...
class AlterationNotifier {
 public:
  ...
  class ObserverBase {
   protected:
    ...
    void attach(AlterationNotifier& nf) { nf.attach(*this); }

    // \brief Detaches the observer into an AlterationNotifier.
    //
    // This member detaches the observer from an AlterationNotifier.
    void detach() { _notifier->detach(*this); }
 ...
 public:
  ...
  AlterationNotifier() : container(0) {}
  ...
  //
  // Sets the container.
  void setContainer(const Container& _container) { container = &_container; }
 ...
 public:
  ...
  void first(Item& item) const { container->first(item); }

  // \brief Next item in the container.
  //
  // Returns the next item in the container. It is
  // for iterate on the container.
  void next(Item& item) const { container->next(item); }

  // \brief Returns the id of the item.
  //
  // Returns the id of the item provided by the container.
  int id(const Item& item) const { return container->id(item); }

  // \brief Returns the maximum id of the container.
  //
  // Returns the maximum id of the container.
  int maxId() const { return container->maxId(Item()); }
