...
class ListDigraphBase {
 ...
 public:
  ...
  void first(Arc& arc) const {
    int n;
    for (n = first_node; n != -1 && _nodes[n].first_out == -1;
         n = _nodes[n].next) {
    }
  ...
  void firstOut(Arc& e, const Node& v) const { e.id = _nodes[v.id].first_out; }
  void nextOut(Arc& e) const { e.id = _arcs[e.id].next_out; }

  void firstIn(Arc& e, const Node& v) const { e.id = _nodes[v.id].first_in; }
  void nextIn(Arc& e) const { e.id = _arcs[e.id].next_in; }
 ...
 protected:
  void changeTarget(Arc e, Node n) {
    ...
    if (_arcs[e.id].prev_in != -1)
      ...
      _nodes[_arcs[e.id].target].first_in = _arcs[e.id].next_in;
  ...
  void changeSource(Arc e, Node n) {
    ...
    if (_arcs[e.id].prev_out != -1)
      ...
      _nodes[_arcs[e.id].source].first_out = _arcs[e.id].next_out;
  ...
  class Snapshot {
   protected:
    ...
    class NodeObserverProxy : public NodeNotifier::ObserverBase {
     ...
     protected:
      virtual void add(const Node& node) { snapshot.addNode(node); }
      virtual void add(const std::vector<Node>& nodes) {
        for (int i = nodes.size() - 1; i >= 0; --i) {
          snapshot.addNode(nodes[i]);
        }
      }
      virtual void erase(const Node& node) { snapshot.eraseNode(node); }
    ...
    class ArcObserverProxy : public ArcNotifier::ObserverBase {
     ...
     protected:
      virtual void add(const Arc& arc) { snapshot.addArc(arc); }
      virtual void add(const std::vector<Arc>& arcs) {
        for (int i = arcs.size() - 1; i >= 0; --i) {
          snapshot.addArc(arcs[i]);
        }
      }
      virtual void erase(const Arc& arc) { snapshot.eraseArc(arc); }
    ...
    void addNode(const Node& node) { added_nodes.push_front(node); }
    ...
    void addArc(const Arc& arc) { added_arcs.push_front(arc); }
    ...
    bool attached() const { return node_observer_proxy.attached(); }
   ...
   public:
    ...
    Snapshot()
        : digraph(0), node_observer_proxy(*this), arc_observer_proxy(*this) {}

    /// \brief Constructor that immediately makes a snapshot.
    ///
    /// This constructor immediately makes a snapshot of the given digraph.
    Snapshot(ListDigraph& gr)
        : node_observer_proxy(*this), arc_observer_proxy(*this) {
      attach(gr);
    }
...
class ListGraphBase {
 ...
 public:
  ...
  class Arc {
   ...
   public:
    operator Edge() const { return id != -1 ? edgeFromId(id / 2) : INVALID; }
  ...
  static bool direction(Arc e) { return (e.id & 1) == 1; }

  static Arc direct(Edge e, bool d) { return Arc(e.id * 2 + (d ? 1 : 0)); }
 ...
 protected:
  void changeV(Edge e, Node n) {
    ...
    if (_arcs[2 * e.id].prev_out != -1) {
      _arcs[_arcs[2 * e.id].prev_out].next_out = _arcs[2 * e.id].next_out;
    } else {
      _nodes[_arcs[(2 * e.id) | 1].target].first_out = _arcs[2 * e.id].next_out;
    }
  ...
  void changeU(Edge e, Node n) {
    ...
    if (_arcs[(2 * e.id) | 1].prev_out != -1) {
      ...
      _nodes[_arcs[2 * e.id].target].first_out = _arcs[(2 * e.id) | 1].next_out;
    }
  ...
  class Snapshot {
   protected:
    ...
    class EdgeObserverProxy : public EdgeNotifier::ObserverBase {
     ...
     protected:
      virtual void add(const Edge& edge) { snapshot.addEdge(edge); }
      virtual void add(const std::vector<Edge>& edges) {
        for (int i = edges.size() - 1; i >= 0; --i) {
          snapshot.addEdge(edges[i]);
        }
      }
      virtual void erase(const Edge& edge) { snapshot.eraseEdge(edge); }
    ...
    void addEdge(const Edge& edge) { added_edges.push_front(edge); }
   ...
   public:
    ...
    /// You have to call save() to actually make a snapshot.
    Snapshot()
        : graph(0), node_observer_proxy(*this), edge_observer_proxy(*this) {}

    /// \brief Constructor that immediately makes a snapshot.
    ///
    /// This constructor immediately makes a snapshot of the given graph.
    Snapshot(ListGraph& gr)
        : node_observer_proxy(*this), edge_observer_proxy(*this) {
      attach(gr);
    }
...
class ListBpGraphBase {
 ...
 public:
  ...
  RedNode  redNode(Edge e) const { return RedNode(_arcs[2 * e.id].target); }
  ...
  void first(RedNode& node) const { node.id = first_red; }

  void next(RedNode& node) const { node.id = _nodes[node.id].partition_next; }

  void first(BlueNode& node) const { node.id = first_blue; }

  void next(BlueNode& node) const { node.id = _nodes[node.id].partition_next; }
  ...
  RedNode addRedNode() {
    ...
    if (first_red != -1)
      _nodes[first_red].partition_prev = n;
    first_red                = n;
  ...
  BlueNode addBlueNode() {
    ...
    if (first_blue != -1)
      _nodes[first_blue].partition_prev = n;
    first_blue               = n;
    _nodes[n].partition_prev = -1;
  ...
  void erase(const Node& node) {
    ...
    if (_nodes[n].partition_next != -1) {
      _nodes[_nodes[n].partition_next].partition_prev =
          _nodes[n].partition_prev;
    }

    if (_nodes[n].partition_prev != -1) {
      _nodes[_nodes[n].partition_prev].partition_next =
          _nodes[n].partition_next;
    } else {
      if (_nodes[n].red) {
  ...
  void clear() {
    _arcs.clear();
    _nodes.clear();
    first_node = first_free_arc = first_red = first_blue = max_red = max_blue =
        first_free_red = first_free_blue = -1;
  }
