...
class FullDigraphBase {
 ...
 public:
  ...
  void first(Arc& arc) const { arc._id = _arc_num - 1; }
...
class FullDigraph : public ExtendedFullDigraphBase {
 ...
 public:
  ...
  Arc arc(Node u, Node v) const { return Parent::arc(u, v); }
...
class FullGraphBase {
 ...
 public:
  ...
  Node u(Edge edge) const { return Node(_uid(edge._id)); }

  Node v(Edge edge) const { return Node(_vid(edge._id)); }

  Node source(Arc arc) const {
    return Node((arc._id & 1) == 1 ? _uid(arc._id >> 1) : _vid(arc._id >> 1));
  }

  Node target(Arc arc) const {
    return Node((arc._id & 1) == 1 ? _vid(arc._id >> 1) : _uid(arc._id >> 1));
  }
  ...
  void first(Arc& arc) const { arc._id = (_edge_num << 1) - 1; }
...
class FullGraph : public ExtendedFullGraphBase {
 ...
 public:
  ...
  Edge edge(Node u, Node v) const { return Parent::edge(u, v); }
...
class FullBpGraphBase {
 ...
 protected:
  FullBpGraphBase() : _red_num(0), _blue_num(0), _node_num(0), _edge_num(0) {}

  void construct(int redNum, int blueNum) {
    _red_num  = redNum;
    _blue_num = blueNum;
    _node_num = redNum + blueNum;
    _edge_num = redNum * blueNum;
  }
 ...
 public:
  ...
  RedNode  redNode(Edge e) const { return RedNode(e._id % _red_num); }
  BlueNode blueNode(Edge e) const {
    return BlueNode(e._id / _red_num + _red_num);
  }

  static bool direction(Arc a) { return (a._id & 1) == 1; }

  static Arc direct(Edge e, bool d) { return Arc(e._id * 2 + (d ? 1 : 0)); }
  ...
  static void next(RedNode& node) { --node._id; }

  void first(BlueNode& node) const {
    if (_red_num == _node_num)
      node._id = -1;
    else
  ...
  void first(Edge& arc) const { arc._id = _edge_num - 1; }

  static void next(Edge& arc) { --arc._id; }
  ...
  bool valid(Arc a) const { return a._id >= 0 && a._id < 2 * _edge_num; }
  bool valid(Edge e) const { return e._id >= 0 && e._id < _edge_num; }

  RedNode redNode(int index) const { return RedNode(index); }

  int index(RedNode n) const { return n._id; }

  BlueNode blueNode(int index) const { return BlueNode(index + _red_num); }

  int index(BlueNode n) const { return n._id - _red_num; }
...
class FullBpGraph : public ExtendedFullBpGraphBase {
 public:
  ...
  Edge edge(const Node& u, const Node& v) const { return Parent::edge(u, v); }

  /// \brief Returns the arc which connects the given nodes.
  ///
  /// Returns the arc which connects the given nodes.
  Arc arc(const Node& u, const Node& v) const { return Parent::arc(u, v); }
