...
class HypercubeGraphBase {
 ...
 public:
  ...
  int dimension(Edge edge) const { return edge._id >> (_dim - 1); }

  int dimension(Arc arc) const { return arc._id >> _dim; }

  static int index(Node node) { return node._id; }
...
class HypercubeGraph : public ExtendedHypercubeGraphBase {
 ...
 public:
  ...
  int dimension() const { return Parent::dimension(); }
  ...
  int dimension(Edge edge) const { return Parent::dimension(edge); }

  /// \brief The dimension id of an arc.
  ///
  /// Gives back the dimension id of the given arc.
  /// It is in the range <tt>[0..dim-1]</tt>.
  int dimension(Arc arc) const { return Parent::dimension(arc); }

  /// \brief The index of a node.
  ///
  /// Gives back the index of the given node.
  /// The lower bits of the integer describes the node.
  static int index(Node node) { return Parent::index(node); }
  ...
  class HyperMap {
   public:
    ...
    HyperMap(
      ...
      LEMON_ASSERT(_values.size() == graph.dimension(), "Wrong size of range");
    }
