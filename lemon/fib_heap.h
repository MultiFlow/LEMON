...
class FibHeap {
 ...
 public:
  ...
  explicit FibHeap(ItemIntMap& map) : _minimum(0), _iim(map), _num() {}
  ...
  void push(const Item& item, const Prio& prio) {
    ...
    if (_num) {
      ...
      if (_comp(prio, _data[_minimum].prio))
        _minimum = i;
    } else {
  ...
  /// \param item The item.
  /// \pre \e item must be in the heap.
  Prio operator[](const Item& item) const { return _data[_iim[item]].prio; }
  ...
  void set(const Item& item, const Prio& prio) {
    int i = _iim[item];
    if (i >= 0 && _data[i].in) {
      if (_comp(prio, _data[i].prio))
        decrease(item, prio);
      if (_comp(_data[i].prio, prio))
        increase(item, prio);
    } else
      push(item, prio);
  }
 ...
 private:
  void balance() {
    ...
    int m = _minimum;
    do {
      if (_comp(_data[s].prio, _data[_minimum].prio))
        _minimum = s;
