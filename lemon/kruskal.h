...
typename disable_if<
    lemon::UndirectedTagIndicator<Digraph>,
...
typename enable_if<
    lemon::UndirectedTagIndicator<Graph>,
    typename In::value_type::second_type>::type
...
template<typename In>
struct MapInputIndicator<In, typename exists<typename In::Value>::type> {
  static const bool value = true;
};
...
template<typename Out>
struct MapOutputIndicator<Out, typename exists<typename Out::Value>::type> {
  static const bool value = true;
};
...
struct KruskalInputSelector<
  ...
  static Value kruskal(const Graph& graph, const In& in, Out& out) {
    ...
    return KruskalOutputSelector<Graph, Sequence, Out>::kruskal(
        graph, seq, out);
  }
};
...
#endif
{
  return _kruskal_bits::KruskalInputSelector<Graph, In, Out>::kruskal(
      graph, in, out);
}
...
kruskal(const Graph& graph, const In& in, const Out& out) {
  return _kruskal_bits::KruskalInputSelector<Graph, In, const Out>::kruskal(
      graph, in, out);
}
