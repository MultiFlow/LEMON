...
class CycleCanceling {
 ...
 private:
  ...
  class StaticVectorMap {
   public:
    ...
    Value& operator[](const Key& key) { return _v[StaticDigraph::id(key)]; }
 ...
 public:
  ...
  Number totalCost() const {
    ...
    for (ArcIt a(_graph); a != INVALID; ++a) {
      int i = _arc_idb[a];
      c += static_cast<Number>(_res_cap[i]) * (-static_cast<Number>(_cost[i]));
    }
    return c;
  }
  ...
  ///
  /// \pre \ref run() must be called before using this function.
  Value flow(const Arc& a) const { return _res_cap[_arc_idb[a]]; }
 ...
 private:
  ...
  ProblemType init() {
    ...
    Circulation<Digraph, ConstMap<Arc, Value>, ValueArcMap, ValueNodeMap> circ(
        _graph, low, cap, sup);
    if (!circ.flowMap(flow).run())
      return INFEASIBLE;
  ...
  bool checkBoundMaps() {
    for (int j = 0; j != _res_arc_num; ++j) {
      if (_forward[j] && _upper[j] < _lower[j])
        return false;
    }
    return true;
  }
  ...
  void start(Method method) {
    ...
    if (method != SIMPLE_CYCLE_CANCELING) {
      ...
      typename BellmanFord<StaticDigraph, CostArcMap>::template SetDistMap<
          CostNodeMap>::Create bf(_sgr, _cost_map);
      bf.distMap(_pi_map);
      bf.init(0);
      bf.start();
    }
  ...
  void startSimpleCycleCanceling() {
    ...
    typedef typename BellmanFord<ResDigraph, CostArcMap>::template SetDistMap<
        CostNodeMap>::template SetPredMap<PredMap>::Create BF;
      ...
      while (!cycle_found) {
        ...
        if (real_iter_num < curr_iter_num) {
          ...
          for (int u = 0; u != _res_node_num; ++u) {
            ...
            int v = u;
            for (; v != -1 && state[v] == 0;
                 v = pred[v] == INVALID ? -1 : rgr.id(rgr.source(pred[v]))) {
              state[v] = id;
            }
            if (v != -1 && state[v] == id) {
  ...
  void startMinMeanCycleCanceling() {
    ...
    typedef typename HowardMmc<StaticDigraph, CostArcMap>::template SetPath<
        SPath>::Create HwMmc;
    typedef
        typename HartmannOrlinMmc<StaticDigraph, CostArcMap>::template SetPath<
            SPath>::Create HoMmc;

    const double HW_ITER_LIMIT_FACTOR    = 1.0;
    const int    HW_ITER_LIMIT_MIN_VALUE = 5;

    const int hw_iter_limit = std::max(
        static_cast<int>(HW_ITER_LIMIT_FACTOR * _node_num),
    ...
    while (true) {
      ...
      if (hw_tc == HwMmc::ITERATION_LIMIT) {
        ...
        if (!(ho_mmc.findCycleMean() && ho_mmc.cycleCost() < 0))
          break;
        ho_mmc.findCycle();
      } else {
        // Find a minimum mean cycle (Howard algorithm)
        if (!(hw_tc == HwMmc::OPTIMAL && hw_mmc.cycleCost() < 0))
          break;
        hw_mmc.findCycle();
      }
    ...
    while (epsilon * _res_node_num >= 1) {
      ...
      for (int start = 0; start != _res_node_num; ++start) {
        ...
        for (; a != last_out && (_res_cap[a] == 0 ||
                                 !tol.negative(_cost[a] + p - pi[_target[a]]));
             ++a)
          ;
        if (a == last_out) {
