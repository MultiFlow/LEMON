...
class HaoOrlin {
 ...
 private:
  ...
  void findMinCutOut() {
    ...
    Node target = _last[_sets.back().back()];
    {
      ...
      while (_highest != _sets.back().end() && !(*_active)[_first[*_highest]]) {
        ++_highest;
      }
    }
      ...
      no_more_push:
        ...
        if (excess != 0) {
          if ((*_next)[n] == INVALID) {
            ...
            new_set->splice(
                new_set->end(), _sets.back(), _sets.back().begin(), ++_highest);
 ...
 public:
  ...
  void calculateOut() { findMinCutOut(); }
