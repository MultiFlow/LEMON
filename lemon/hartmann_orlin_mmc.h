...
class HartmannOrlinMmc {
 ...
 public:
  ...
  template<typename T>
  struct SetLargeCost : public HartmannOrlinMmc<GR, CM, SetLargeCostTraits<T>> {
  ...
  /// and it must have an \c addFront() function.
  template<typename T>
  struct SetPath : public HartmannOrlinMmc<GR, CM, SetPathTraits<T>> {
 ...
 public:
  ...
  bool findCycleMean() {
    ...
    for (int comp = 0; comp < _comp_num; ++comp) {
      if (!initComponent(comp))
        continue;
      processRounds();

      // Update the best cycle (global minimum mean cycle)
      if (_curr_found &&
          (!_best_found || _curr_cost * _best_size < _best_cost * _curr_size)) {
        _best_found = true;
  ...
  /// using this function.
  Cost cycleCost() const { return static_cast<Cost>(_best_cost); }
  ...
  /// \pre \ref run() or \ref findCycleMean() must be called before
  /// using this function.
  int cycleSize() const { return _best_size; }
