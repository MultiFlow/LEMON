...
class CompactDigraphBase {
 ...
 public:
  ...
  void firstOut(Arc& e, const Node& n) const {
    ...
    if (e.id == node_first_out[n.id + 1])
      e = INVALID;
  }
  void nextOut(Arc& e) const {
    ++e.id;
    if (e.id == node_first_out[e.source + 1])
      e = INVALID;
  }
  ...
  Arc        arcFromId(int id) const {
    int* l =
        std::upper_bound(node_first_out, node_first_out + node_num, id) - 1;
 ...
 public:
  ...
  void build(int n, ArcListIterator first, ArcListIterator last) {
    ...
    LEMON_ASSERT(first == last, "Wrong arc list for CompactDigraph::build()");
    node_first_out[node_num] = arc_num;
  }
