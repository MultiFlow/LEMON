...
int GlpkBase::_addRow(Value lo, ExprIterator b, ExprIterator e, Value up) {
  int i = glp_add_rows(lp, 1);
  ...
  glp_set_mat_row(lp, i, values.size() - 1, &indexes.front(), &values.front());
  return i;
}
...
void GlpkBase::_setColCoeffs(int ix, ExprIterator b, ExprIterator e) {
  std::vector<int>   indexes;
  std::vector<Value> values;
  ...
  glp_set_mat_col(lp, ix, values.size() - 1, &indexes.front(), &values.front());
}
...
GlpkLp::GlpkLp() : LpBase(), LpSolver(), GlpkBase() {
  presolver(false);
}
...
GlpkLp* GlpkLp::newSolver() const {
  return new GlpkLp;
}
GlpkLp* GlpkLp::cloneSolver() const {
  return new GlpkLp(*this);
}

const char* GlpkLp::_solverName() const {
  return "GlpkLp";
}
...
GlpkLp::Value GlpkLp::_getPrimalRay(int i) const {
  if (_primal_ray.empty()) {
    ...
    if (index != 0) {
      ...
      LEMON_ASSERT(
          (index <= row_num ? glp_get_row_stat(lp, index)
      ...
      int                ray_length = glp_eval_tab_col(
          lp, index, &ray_indexes.front(), &ray_values.front());
...
GlpkMip::GlpkMip() : LpBase(), MipSolver(), GlpkBase() {}
...
GlpkMip* GlpkMip::newSolver() const {
  return new GlpkMip;
}
GlpkMip* GlpkMip::cloneSolver() const {
  return new GlpkMip(*this);
}

const char* GlpkMip::_solverName() const {
  return "GlpkMip";
}
