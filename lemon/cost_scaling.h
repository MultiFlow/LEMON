...
class CostScaling {
 ...
 public:
  ...
  /// \c Cost must be convertible to \c LargeCost.
  template<typename T>
  struct SetLargeCost : public CostScaling<GR, V, C, SetLargeCostTraits<T>> {
 ...
 public:
  ...
  Number totalCost() const {
    ...
    for (ArcIt a(_graph); a != INVALID; ++a) {
      int i = _arc_idb[a];
      c += static_cast<Number>(_res_cap[i]) * (-static_cast<Number>(_scost[i]));
    }
    return c;
  }
 ...
 private:
  ...
  void start(Method method) {
    ...
    if (!optimal) {
      ...
      typename BellmanFord<StaticDigraph, LargeCostArcMap>::Create bf(
          sgr, cost_map);
      bf.init(0);
      bf.start();
  ...
  void startAugment(int max_length) {
    ...
    const int    global_update_skip     = static_cast<int>(
        GLOBAL_UPDATE_FACTOR * (_res_node_num + _sup_node_num * _sup_node_num));
    ...
    int        eps_phase_cnt = 0;
    for (; _epsilon >= 1;
         _epsilon = _epsilon < _alpha && _epsilon > 1 ? 1 : _epsilon / _alpha) {
      ++eps_phase_cnt;
      ...
      next_node:
        ...
        if (_excess[n] > 0) {
          for (a = _next_out[n]; a != last_out; ++a) {
            if (_res_cap[a] > 0 && _cost[a] + pi_n - _pi[_target[a]] < 0) {
              ...
              for (int ta = _next_out[t]; ta != last_out_t; ++ta) {
                if (_res_cap[ta] > 0 && _cost[ta] + pi_t - _pi[_target[ta]] < 0)
                  ahead += _res_cap[ta];
                if (ahead >= delta)
                  break;
              }
        ...
        if (_excess[n] > 0 || hyper[n]) {
          min_red_cost =
              hyper[n] ? -hyper_cost[n] : std::numeric_limits<LargeCost>::max();
