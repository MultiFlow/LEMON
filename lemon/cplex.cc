...
CplexBase::CplexBase(const CplexEnv& env) : LpBase(), _env(env) {
  int status;
  _prob = CPXcreateprob(cplexEnv(), &status, "Cplex problem");
  messageLevel(MESSAGE_NOTHING);
}

CplexBase::CplexBase(const CplexBase& cplex) : LpBase() {
  int status;
...
int CplexBase::_addRow(Value lb, ExprIterator b, ExprIterator e, Value ub) {
  int i = CPXgetnumrows(cplexEnv(), _prob);
...
void CplexBase::_getColName(int col, std::string& name) const {
  ...
  char*             cname;
  int               tmp;
  CPXgetcolname(cplexEnv(), _prob, &cname, &buf.front(), size, &tmp, col, col);
  name = cname;
}
...
int CplexBase::_colByName(const std::string& name) const {
  int index;
  if (CPXgetcolindex(
          cplexEnv(), _prob, const_cast<char*>(name.c_str()), &index) == 0) {
    return index;
  }
  return -1;
}
...
void CplexBase::_getRowName(int row, std::string& name) const {
  ...
  char*             cname;
  int               tmp;
  CPXgetrowname(cplexEnv(), _prob, &cname, &buf.front(), size, &tmp, row, row);
  name = cname;
}
...
void CplexBase::_setRowCoeffs(int i, ExprIterator b, ExprIterator e) {
  std::vector<int>   indices;
...
void CplexBase::_setObjCoeffs(ExprIterator b, ExprIterator e) {
  ...
  CPXchgobj(
      cplexEnv(), _prob, values.size(), &indices.front(), &values.front());
}
...
void CplexBase::_applyMessageLevel() {
  CPXsetintparam(
      cplexEnv(), CPX_PARAM_SCRIND, _message_enabled ? CPX_ON : CPX_OFF);
}
...
CplexLp::CplexLp() : LpBase(), LpSolver(), CplexBase() {}

CplexLp::CplexLp(const CplexEnv& env) : LpBase(), LpSolver(), CplexBase(env) {}
...
CplexLp* CplexLp::newSolver() const {
  return new CplexLp;
}
CplexLp* CplexLp::cloneSolver() const {
  return new CplexLp(*this);
}

const char* CplexLp::_solverName() const {
  return "CplexLp";
}
...
CplexMip::CplexMip() : LpBase(), MipSolver(), CplexBase() {
#if CPX_VERSION < 800
...
CplexMip* CplexMip::newSolver() const {
  return new CplexMip;
}
CplexMip* CplexMip::cloneSolver() const {
  return new CplexMip(*this);
}

const char* CplexMip::_solverName() const {
  return "CplexMip";
}
