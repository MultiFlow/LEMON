 ...
 public:
  ...
  EdmondsKarp(
      ...
      : _graph(digraph),
        ...
        _flow_value() {
    LEMON_ASSERT(
        _source != _target, "Flow source and target are the same nodes.");
  }

  /// \brief Destructor.
  ///
  /// Destructor.
  ~EdmondsKarp() { destroyStructures(); }
  ...
  bool checkedInit(const FlowMap& flowMap) {
    ...
    for (ArcIt it(_graph); it != INVALID; ++it) {
      if (_tolerance.less((*_flow)[it], 0))
        return false;
      if (_tolerance.less((*_capacity)[it], (*_flow)[it]))
        return false;
    }
    _flow_value = 0;
    for (OutArcIt jt(_graph, _source); jt != INVALID; ++jt) {
  ...
  /// using this function.
  Value flowValue() const { return _flow_value; }
