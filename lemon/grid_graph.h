...
class GridGraphBase {
 ...
 public:
  Node operator()(int i, int j) const {
    LEMON_DEBUG(
        0 <= i && i < _width && 0 <= j && j < _height, "Index out of range");
    return Node(i + j * _width);
  }

  int col(Node n) const { return n._id % _width; }

  int row(Node n) const { return n._id / _width; }
  ...
  Node source(Arc arc) const { return (arc._id & 1) == 1 ? u(arc) : v(arc); }

  Node target(Arc arc) const { return (arc._id & 1) == 1 ? v(arc) : u(arc); }
  ...
  Edge findEdge(Node u, Node v, Edge prev = INVALID) const {
    ...
    if (v._id > u._id) {
      ...
      if (v._id - u._id == 1 && u._id % _width < _width - 1) {
        return Edge(
            u._id / _width * (_width - 1) + u._id % _width + _edge_limit);
      }
    } else {
      ...
      if (u._id - v._id == 1 && v._id % _width < _width - 1) {
        return Edge(
            v._id / _width * (_width - 1) + v._id % _width + _edge_limit);
      }
    }
    return INVALID;
  }
  ...
  static bool direction(Arc arc) { return (arc._id & 1) == 1; }
  ...
  void first(Edge& edge) const { edge._id = _edge_num - 1; }

  static void next(Edge& edge) { --edge._id; }

  void first(Arc& arc) const { arc._id = 2 * _edge_num - 1; }
  ...
  void firstOut(Arc& arc, const Node& node) const {
    if (node._id % _width < _width - 1) {
      arc._id =
          (_edge_limit + node._id % _width + (node._id / _width) * (_width - 1))
              << 1 |
          1;
      return;
    }
  ...
  void nextOut(Arc& arc) const {
    ...
    if ((arc._id & 1) == 1) {
      ...
      if (nid % _width > 0) {
        arc._id =
            (_edge_limit + nid % _width + (nid / _width) * (_width - 1) - 1)
            << 1;
        return;
      }
      if (nid >= _width) {
  ...
  void nextIn(Arc& arc) const {
    ...
    if ((arc._id & 1) == 0) {
      ...
      if (nid % _width > 0) {
        arc._id = (_edge_limit + nid % _width + (nid / _width) * (_width - 1) -
                   1) << 1 |
                  1;
        return;
      }
  ...
  Arc right(Node n) const {
    if (n._id % _width < _width - 1) {
      return Arc(
          ((_edge_limit + n._id % _width + (n._id / _width) * (_width - 1))
           << 1) |
          1);
    } else {
      return INVALID;
  ...
  Arc left(Node n) const {
    if (n._id % _width > 0) {
      return Arc(
          (_edge_limit + n._id % _width + (n._id / _width) * (_width - 1) - 1)
          << 1);
    } else {
      return INVALID;
    }
 ...
 public:
  ...
  class IndexMap {
   public:
    ...
    Value operator[](Key key) const { return _graph.pos(key); }
  ...
  class ColMap {
   public:
    ...
    /// \brief The subscript operator
    Value operator[](Key key) const { return _graph.col(key); }
  ...
  class RowMap {
   public:
    ...
    /// \brief The subscript operator
    Value operator[](Key key) const { return _graph.row(key); }
  ...
  Node operator()(int i, int j) const { return Parent::operator()(i, j); }

  /// \brief The column index of the node.
  ///
  /// Gives back the column index of the node.
  int col(Node n) const { return Parent::col(n); }

  /// \brief The row index of the node.
  ///
  /// Gives back the row index of the node.
  int row(Node n) const { return Parent::row(n); }

  /// \brief The position of the node.
  ///
  /// Gives back the position of the node, ie. the <tt>(col,row)</tt> pair.
  dim2::Point<int> pos(Node n) const { return Parent::pos(n); }

  /// \brief The number of the columns.
  ///
  /// Gives back the number of the columns.
  int width() const { return Parent::width(); }

  /// \brief The number of the rows.
  ///
  /// Gives back the number of the rows.
  int height() const { return Parent::height(); }

  /// \brief The arc goes right from the node.
  ///
  /// Gives back the arc goes right from the node. If there is not
  /// outgoing arc then it gives back INVALID.
  Arc right(Node n) const { return Parent::right(n); }

  /// \brief The arc goes left from the node.
  ///
  /// Gives back the arc goes left from the node. If there is not
  /// outgoing arc then it gives back INVALID.
  Arc left(Node n) const { return Parent::left(n); }

  /// \brief The arc goes up from the node.
  ///
  /// Gives back the arc goes up from the node. If there is not
  /// outgoing arc then it gives back INVALID.
  Arc up(Node n) const { return Parent::up(n); }

  /// \brief The arc goes down from the node.
  ///
  /// Gives back the arc goes down from the node. If there is not
  /// outgoing arc then it gives back INVALID.
  Arc down(Node n) const { return Parent::down(n); }

  /// \brief Index map of the grid graph
  ///
  /// Just returns an IndexMap for the grid graph.
  IndexMap indexMap() const { return IndexMap(*this); }

  /// \brief Row map of the grid graph
  ///
  /// Just returns a RowMap for the grid graph.
  RowMap rowMap() const { return RowMap(*this); }

  /// \brief Column map of the grid graph
  ///
  /// Just returns a ColMap for the grid graph.
  ColMap colMap() const { return ColMap(*this); }
};
